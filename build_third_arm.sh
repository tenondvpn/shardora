...
export CC=/usr/local/gcc15/bin/gcc
export CXX=/usr/local/gcc15/bin/g++
export LD_LIBRARY_PATH=/usr/local/gcc15/lib64:$LD_LIBRARY_PATH
...
cd $SRC_PATH
cd third_party/openssl/ && git checkout 7b371d8 && ./Configure CFLAGS="-D_GNU_SOURCE -DOPENSSL_NO_ASM" --prefix=$SRC_PATH/third_party/ && make -j${nproc} && make install
...
cd $SRC_PATH
cd third_party/oqs && git checkout 94b421e && cmake -S . -B build_release -DOQS_USE_OPENSSL=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_INSTALL_PREFIX=$SRC_PATH/third_party/ && cd build_release && make -j8 && make install
...
cd $SRC_PATH
if [ -f "$SRC_PATH/third_party/libbls/deps/argtable2/src/arg_int.c" ]; then
    bash "$SRC_PATH/scripts/patch_argtable2_arg_int.sh" "$SRC_PATH/third_party/libbls/deps/argtable2/src/arg_int.c"
fi
