...
server1=10.101.20.35
server2=10.101.20.31
server3=10.101.20.32
server4=10.101.20.33
target=$1
no_build=$2
...
cd /root && sh -x fetch.sh 127.0.0.1 ${server0} $!@#$%^ r1 s3_3 s3_8 s3_13 s3_18 s3_23 s3_28 s3_33 s3_38 s3_43 s3_48
# EOF
...
cd /root && sh -x fetch.sh ${server0} ${server1} '!@#$%^' r2 s3_4 s3_9 s3_14 s3_19 s3_24 s3_29 s3_34 s3_39 s3_44 s3_49
EOF
) &
...
rm -rf /root/zjnodes;
sshpass -p '!@#$%^' scp -o StrictHostKeyChecking=no root@"${server0}":/root/fetch.sh /root/
cd /root && sh -x fetch.sh ${server0} ${server2} '!@#$%^' s3_2 s3_7 s3_12 s3_17 s3_22 s3_27 s3_32 s3_37 s3_42 s3_47
EOF
) &
...
rm -rf /root/zjnodes;
sshpass -p '!@#$%^' scp -o StrictHostKeyChecking=no root@"${server0}":/root/fetch.sh /root/
cd /root && sh -x fetch.sh ${server0} ${server3} '!@#$%^' s3_1 s3_6 s3_11 s3_16 s3_21 s3_26 s3_31 s3_36 s3_41 s3_46
EOF
) &
...
rm -rf /root/zjnodes;
sshpass -p '!@#$%^' scp -o StrictHostKeyChecking=no root@"${server0}":/root/fetch.sh /root/
cd /root && sh -x fetch.sh ${server0} ${server4} '!@#$%^' r3 s3_5 s3_10 s3_15 s3_20 s3_25 s3_30 s3_35 s3_40 s3_45 s3_50
EOF
) &
...
for node in r2 s3_4 s3_9 s3_14 s3_19 s3_24 s3_29 s3_34 s3_39 s3_44 s3_49; do \
...
echo "[$server0]"
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/gcc-8.3.0/lib64
for node in s3_3 s3_8 s3_13 s3_18 s3_23 s3_28 s3_33 s3_38 s3_43 s3_48; do
cd /root/zjnodes/$node/ && nohup ./zjchain -f 0 -g 0 $node> /dev/null 2>&1 &
done
