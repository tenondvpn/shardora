...
cd /root && sh -x fetch.sh 127.0.0.1 ${server0} $!@#$%^ r1 r2 s3_1 s3_2 s3_3 s4_1 s4_2 s4_3 s4_4
# EOF
...
rm -rf /root/zjnodes;
sshpass -p '!@#$%^' scp -o StrictHostKeyChecking=no root@"${server0}":/root/fetch.sh /root/
cd /root && sh -x fetch.sh ${server0} ${server2} '!@#$%^' s3_6 s4_7
EOF
) &
