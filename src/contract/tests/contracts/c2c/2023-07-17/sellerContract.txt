...
contract SellerContract{
    ...
    struct MakerOrder {
        ...
        MakerOrderState makerOrderState;         //挂单订单状态：活跃状态(即挂单列表中可见) 和 非活跃状态
    }
    ...
    function addMakerOrder(address makerAddress, uint64 price, uint256 totalAmount) public returns (uint){ //加锁或加可重入锁？
        if(!precondition4MakerOrder(price, totalAmount)){ //前置条件检查
            return 0;
        }
    ...
    function addTakerOrder(uint64 makerOrderId, address takerAddress, uint256 amount) public returns (uint){   //加锁或加可重入锁？
        if(precondition4TakerOrder(makerOrderId, amount) == 0){ //前置条件检查
            return 0;
        }
        if(true/*买家保证金往合约地址transfer转币成功*/){  //从个人钱包takerAddress往合约地址contractAddress转币
            ...
            uint256 totalValue = calculateTotalValue(price, amount); //4、计算实际支付总金额totalValue （误差控制在[-50, 50]美分）
    ...
    function sellerPaymentConfirm (uint64 makerOrderId, uint64 takerOrderId) public returns (bool){
	   if(!preconditionBeforeHandle(makerOrderId, takerOrderId)){ 
        ...
        if(needRemoveSpecificOrder(makerOrderId)){  //移除旧的挂单
            removeOrderByOrderId(makerOrderId);
        }
    }
    ...
    function precondition4TakerOrder(uint64 makerOrderId, uint256 amount) internal view returns (uint){
        ...
        if(!isExistMakerOrder(makerOrderId)){ //3、未查询到对应的挂单
            return 0;
        }
        MakerOrder storage makerOrder = getMakerOrder(makerOrderId);
    ...
    function precondition4BuyerPayment(uint64 makerOrderId, uint64 takerOrderId) internal view returns (bool){
        if(!isExistTakerOrder(makerOrderId, takerOrderId)){  //1、确认吃单订单存在
            return false;
        }
        TakerOrder storage takerOrder = getTakerOrder(makerOrderId, takerOrderId);
        if(takerOrder.paymentState == BuyerPaymentState.payment){ //2、买家操作过"已汇款"
            return false;
        }
        return true;
    }
    ...
    function precondition4PaymentConfirm(uint64 makerOrderId, uint64 takerOrderId) internal view returns (bool){
        if(!preconditionBeforeHandle(makerOrderId, takerOrderId)){  //1、操作前置条件检查
            return false;
        }
        TakerOrder storage takerOrder = getTakerOrder(makerOrderId, takerOrderId);
        if(takerOrder.paymentConfirmState == SellerPaymentConfirmState.paymentConfirmed){ //2、卖家操作过"确认收款"
            return false;
        }
        return true;
    }

    //订单取消前的条件检查，失败返回false，成功返回true
    function precondition4CancelOrder(uint64 makerOrderId, uint64 takerOrderId, CancelState cancelState) internal view returns (bool){
        ...
        TakerOrder storage takerOrder = getTakerOrder(makerOrderId, takerOrderId);
        if(takerOrder.cancelState == cancelState){ //2、已取消过，直接返回true
            return false;
        }
        return true;
    }

      //操作前置条件检查，失败返回false，成功返回true
    function preconditionBeforeHandle(uint64 makerOrderId, uint64 takerOrderId) internal view returns (bool){
        if(!isExistTakerOrder(makerOrderId, takerOrderId)){  //1、确认吃单订单存在
            return false;
        }
        TakerOrder storage takerOrder = getTakerOrder(makerOrderId, takerOrderId);
        if(takerOrder.takerOrderState == TakerOrderState.Completed){ //2、已完结
            return false;
        }
        return true;
    }
    ...
    function isTakerOrderOver(uint64 makerOrderId, uint64 takerOrderId) public view returns(bool){
        if(isExistTakerOrder(makerOrderId, takerOrderId)){
            TakerOrder storage takerOrder = getTakerOrder(makerOrderId ,takerOrderId);
            if(takerOrder.cancelState == CancelState.sellerCanceled){
                return true;
            }
            if(takerOrder.paymentConfirmState == SellerPaymentConfirmState.paymentConfirmed){
                return true;
            }
        }
        return false;
    ...
    //判断是否需要移除优先级最高的挂单(价格最高)，是返回true，否则返回false
    ...
    //删除优先级最高挂单(即单价最高，存在单价相同的话，删除更早的)，同时赎回该挂单所有未锁定的币
    function removeOrder() internal returns (uint64){
     ...
     //根据订单号删除挂单，同时赎回该挂单所有未锁定的币
    function removeOrderByOrderId(uint64 orderId) internal returns (uint64){
        MakerOrder storage makerOrder = getMakerOrder(orderId);
        withdrawUnlock(orderId, makerOrder.unlockedAmount);   //赎回所有未锁定的币
        makerOrder.makerOrderState = MakerOrderState.Inactive; //挂单不在允许出售
        //deleteMakerOrder(orderId);                            //删除挂单列表中的挂单
    }
