...
contract BuyerContract{
    ...
    Heap.Data public priorityQueue;  //优先级由单价和订单号共同决定（单价越低优先级越高，单价相同时，订单号越小优先级越高），另外订单号映射到挂单列表中挂单。
    ...
    struct MakerOrder {
        ...
        uint256 totalAmount;    //总购量  （挂单时传入，始终不变）
        uint256 availableAmount;//可购数量（根据未锁定保证金，计算得到）
        uint256 filledAmount;   //已购数量（双方交易成功后，累计的已购数量）
    ...
    struct TakerOrder {
        ...
        string nickname;         //昵称
        uint256 price;           //单价  
        uint256 amount;          //交易金数量
        uint256 totalValue;      //实际支付金额（不完全等于单价乘以交易数量）
    ...
    function addMakerOrder(address makerAddress, uint64 price, uint256 totalAmount) public returns (uint){//加锁或加可重入锁？
        ...
        if(true/*买家保证金往合约地址transfer转币成功*/){
            ...
            priorityQueue.insertWithId(pricePriorityInterconvert(price), orderId);//新建挂单加入优先队列(价格，订单号)
            return orderId;
        }else{
            return 0;
        }
    ...
    function addTakerOrder(uint64 makerOrderId, address takerAddress, uint256 amount) public returns (uint){//加锁或加可重入锁？
        ...
        if(true/*卖家交易金和保证金往合约地址transfer转币成功*/){  //从个人钱包takerAddress往合约地址contractAddress转币
    ...
    function sellerPaymentConfirm (uint64 makerOrderId, uint64 takerOrderId) public returns (bool){
	   if(!precondition4PaymentConfirm(makerOrderId, takerOrderId)){ 
        ...
        if(true/*合约地址往买家地址transfer转币成功(含自己释放的保证金)*/){//同时合约地址往卖家地址转币（保证金）
            ...
            refreshMakerOrderAmount(makerOrderId, amount, ActionState.DealSuccess);  //交易成功买家不增加未锁定的币数量
        }
        removeOrderIfNecessary(makerOrderId);
        return true;
    }
    ...
    function precondition4MakerOrder(uint64 price, uint256 totalAmount) internal view returns (bool){
        ...
        if(size >= maxSizeThreshold){
            ...
            uint64 minPrice = pricePriorityInterconvert(head.priority);
            if(price < minPrice){                    //3、挂单列表已满，单价比优先队列中最低单价还小，不允许出售
                return false;
            }
        }
    ...
    function precondition4TakerOrder(uint64 makerOrderId, uint256 amount) internal view returns (bool){
        ...
        if(!isExistMakerOrder(makerOrderId)){ //3、未查询到对应的挂单， 挂单列表不可见？
            return false;
        }
        MakerOrder storage makerOrder = getMakerOrder(makerOrderId);
        ...
        if(amount > availableAmount){ //5、出售数量大于挂单可购数量
             return false; 
        }
        return true;
    }
    ...
    function precondition4WithdrawUnlock(uint64 orderId, uint256 amount) internal view returns (bool){
        ...
        if(amount > unLockedAmount){ //赎回币数量大于未锁定币数量  (币不够)
            return false;
        }
        return true;
    }
    ...
    //是否移除优先级最高的挂单(价格最低)，是返回true，否则返回false
    ...
    //删除优先级最高挂单(即单价最低，存在单价相同的话，删除更早的)
    ...
    function removeOrderMandatory(uint64 orderId) internal returns (uint64){
     ...
     //根据情况决定移除挂单（比如：可购币数量太少了），同时赎回该挂单所有未锁定的币
    ...
    function refreshMakerOrderAmount(uint64 orderId, uint256 amount, ActionState action) internal{
        ...
        if(action == ActionState.MakerOrder){
            unlockAmount = calculateBuyerMortgage(orderId, amount);                 //1、挂单，保证金未锁定，增加未锁定量
        }else if(action == ActionState.TakerOrder){
            unlockAmount = 0 - calculateBuyerMortgage(orderId, amount);             //2、吃单，锁定买家保证金，减少未锁定量
        }else if(action == ActionState.CancelOrder){
            unlockAmount = calculateBuyerMortgage(orderId, amount);                 //3、取消，释放买家保证金，增加未锁定量
        }else if(action == ActionState.WithdrawUnlock){
            unlockAmount = 0 - amount;                                             //4、赎回，               直接减少未锁定量
        }else if(action == ActionState.DealSuccess){
            unlockAmount = calculateBuyerMortgage(orderId, 0);                     //5、交易成功，释放买家保证金， 但未增加未锁定量（直接赎回个人钱包）
        }
    ...
    function refreshAvailableAmount(uint64 orderId) internal returns (uint){
        MakerOrder storage makerOrder = getMakerOrder(orderId);
        uint unLockedAmount = makerOrder.unlockedAmount;
        makerOrder.availableAmount = unLockedAmount * 100 / (makerOrder.buyerMortgagePercentage);
    ...
    function calculateAvailableAmount(uint64 orderId) internal view returns(uint256){
        MakerOrder storage makerOrder = getMakerOrder(orderId);
        uint256 availableAmount = makerOrder.totalAmount;
    ...
    function pricePriorityInterconvert(uint64 priceOrPriority) internal pure returns (uint64){
        priceOrPriority = (2**64 - 1) - priceOrPriority; //最大值未使用 type(uint256).max，因为版本太低
