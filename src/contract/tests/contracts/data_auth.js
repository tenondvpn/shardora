...
function param_contract(tx_type, gid, to, amount, gas_limit, gas_price, contract_bytes, input, prefund, key, value) {
    ...
    var message_buf = Buffer.concat([Buffer.from(gid, 'hex'), Buffer.from(frompk, 'hex'), Buffer.from(to, 'hex'),
        ...
        Buffer.from(input, 'hex'), prefund_buf, Buffer.from(key), Buffer.from(value)]);
...
function param_contract2(tx_type, gid, to, amount, gas_limit, gas_price, contract_bytes, input, prefund) {
    var gid = GetValidHexString(Secp256k1.uint256(randomBytes(32)));
...
function create_tx(to, amount, gas_limit, gas_price, prefund, tx_type) {
    var gid = GetValidHexString(Secp256k1.uint256(randomBytes(32)));
    var frompk = '04' + self_public_key.x.toString(16) + self_public_key.y.toString(16);
    const MAX_UINT32 = 0xFFFFFFFF;
    var amount_buf = new Buffer(8);
...
function create_tx2(to, amount, gas_limit, gas_price, prefund, tx_type) {
    var gid = GetValidHexString(Secp256k1.uint256(randomBytes(32)));
    var frompk = '04' + self_public_key2.x.toString(16) + self_public_key2.y.toString(16);
    const MAX_UINT32 = 0xFFFFFFFF;
    var amount_buf = new Buffer(8);
...
function Prefund(prefund) {
    var contract_address = fs.readFileSync('contract_address', 'utf-8');
    var data = create_tx(contract_address, 0, 100000, 1, prefund, 7);
    PostCode(data);
}

function Prefund2(prefund) {
    var contract_address = fs.readFileSync('contract_address', 'utf-8');
    var data = create_tx2(contract_address, 0, 100000, 1, prefund, 7);
    PostCode(data);
}
