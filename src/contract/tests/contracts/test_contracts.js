...
function Prefund(str_prikey, prefund, contract_address) {
...
async function SetManagerPrefund(contract_address) {
    // 为管理账户设置prefund
    Prefund("cefc2c33064ea7691aee3e5e4f7842935d26f3ad790d81cf015e79b78958e848", 1000000000000, contract_address);
    ...
    var check_count = 1;
    var cmd = `clickhouse-client --host 127.0.0.1 --port 9000 -q "select count(distinct(user)) from shardora_ck_prefund_table where contract='${contract_address}' and user in (${check_accounts_str});"`;
    ...
    while (try_times < 30) {
        try {
            ...
            if (stdout.trim() == check_count.toString()) {
                console.error(`contract prefund success: ${stdout}, contract_address: ${contract_address}`);
                break;
            }

            console.log(`${cmd} contract prefund failed error: ${stderr} count: ${stdout} ${contract_address}`);
        } catch (error) {
            console.log(error);
        }
    ...
    if (try_times >= 30) {
        console.error(`contract prefund failed! ${contract_address}`);
        return;
    }
}
    ...
    exec('solc --bin ./proxy_reencyption.sol', async (error, stdout, stderr) => {
        ...
        // console.log(`solc bin codes: ${out_lines[3]}`);
        {
            ...
            // 转账到管理账户，创建合约
            {
                ...
                var contract_cmd = `clickhouse-client --host 127.0.0.1 --port 9000 -q  "SELECT to FROM shardora_ck_account_key_value_table where type = 6 and key in ('5f5f6b437265617465436f6e74726163744279746573436f6465',  '5f5f6b437265617465436f6e74726163744279746573436f6465') and to='${contract_address}' limit 1;"`
                var try_times = 0;
                // 检查合约是否创建成功
                const execPromise = util.promisify(exec);
                while (try_times < 30) {
