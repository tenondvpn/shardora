...
function InitC2cEnv(key, value) {
    const { exec } = require('child_process');
    exec('solc --bin ./pki_cl.sol', async (error, stdout, stderr) => {
        if (error) {
...
var sks = [
    "762f480a38f23a528b3ce6c1314c53711d4a9c40",
...
if (args[0] == 3) {
    EncKeyGen("cliege", "cliege", pki_count.toString() + ";" + ib_count.toString() + ";" + id, id);
}

if (args[0] == 4) {
    DecKeyGen("clidge", "clidge", pki_count.toString() + ";" + ib_count.toString() + ";" + id, id);
}

if (args[0] == 5) {
    Enc("clienc", "clienc", id + ";" + args[2], id);
}

if (args[0] == 6) {
    Dec("clidec", "clidec", id + ";" + args[2], id);
}
