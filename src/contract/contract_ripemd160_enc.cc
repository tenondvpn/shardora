...
Ripemd160Enc::Ripemd160Enc(const std::string& create_address)
        : ContractInterface(create_address) {}

Ripemd160Enc::~Ripemd160Enc() {}
...
void Ripemd160Enc::SaveCrs(const CRS& crs, std::string* val) {
    char data[10240] = {0};
...
void Ripemd160Enc::LoadCrs(const std::string& val, CRS& crs) {
...
int Ripemd160Enc::CreateArsKeys(
        const CallParameters& param, 
        const std::string& key, 
        const std::string& value) {
    ContractArs ars;
...
int Ripemd160Enc::AggSignAndVerify(
        const CallParameters& param, 
        const std::string& key, 
        const std::string& value) {
        // 聚合签名生成
...
int Ripemd160Enc::CheckDecrytParamsValid(
        const CallParameters& param,
        uint64_t gas,
        const std::string& origin_address,
        evmc_result* res) {
...
int Ripemd160Enc::AddReEncryptionParam(
        const CallParameters& param,
        uint64_t gas,
        const std::string& origin_address,
        evmc_result* res) {
...
void Ripemd160Enc::AddAllParams(
...
int Ripemd160Enc::TestPbc(const std::string& param) {
