...
//     std::string all_ids;
//     for (uint32_t idx = 0; idx < genesis_nodes.size(); ++idx) {
//         all_ids += genesis_nodes[idx]->id;
//     }

//     nlohmann::json bls_save_json;
//     std::string all_id_hash = common::Encode::HexEncode(common::Hash::keccak256(all_ids));
//     FILE* ex_fd = NULL;// fopen((std::string("/var/") + all_id_hash).c_str(), "r");
//     if (ex_fd != NULL) {
//         char data[10240];
//         std::string all_data;
//         while (!feof(ex_fd)) {
//             auto res = fread(data, 1, sizeof(data), ex_fd);
//             all_data += std::string(data, res);
//         }

//         fclose(ex_fd);
//         bls_save_json = nlohmann::json::parse(all_data);
//         db::DbWriteBatch dbbatch;
//         auto common_public_key = libff::alt_bn128_G2::zero();
//         for (uint32_t idx = 0; idx < genesis_nodes.size(); ++idx) {
//             auto id = common::Encode::HexDecode(bls_save_json[idx]["id"]);
//             assert(id == genesis_nodes[idx]->id);
//             bls::protobuf::LocalPolynomial local_poly;
//             auto poly = common::Encode::HexDecode(bls_save_json[idx]["poly"]);
//             local_poly.ParseFromString(poly);
//             auto check_hash = common::Encode::HexDecode(bls_save_json[idx]["check_hash"]);
//             auto verify_g2 = common::Encode::HexDecode(bls_save_json[idx]["verify_g2"]);
//             bls::protobuf::JoinElectInfo join_info;
//             auto join_info_str = common::Encode::HexDecode(bls_save_json[idx]["join_info"]);
//             join_info.ParseFromString(join_info_str);
//             std::shared_ptr<security::Security> secptr = std::make_shared<security::Ecdsa>();
//             secptr->SetPrivateKey(genesis_nodes[idx]->prikey);
//             prefix_db_->SaveLocalPolynomial(secptr, secptr->GetAddress(), local_poly, false, dbbatch);
//             prefix_db_->AddBlsVerifyG2(secptr->GetAddress(), join_info.g2_req(), dbbatch);
//             prefix_db_->SaveTemporaryKv(genesis_nodes[idx]->check_hash, join_info.SerializeAsString(), dbbatch);
//             auto bls_prikey = common::Encode::HexDecode(bls_save_json[idx]["bls_prikey"]);
//             prefix_db_->SaveBlsPrikey(
//                 elect_height,
//                 sharding_id,
//                 genesis_nodes[idx]->id,
//                 bls_prikey);
//             auto x_c0 = libff::alt_bn128_Fq(bls_save_json[idx]["verification_x_c0"].get<std::string>().c_str());
//             auto x_c1 = libff::alt_bn128_Fq(bls_save_json[idx]["verification_x_c1"].get<std::string>().c_str());
//             auto x_coord = libff::alt_bn128_Fq2(x_c0, x_c1);
//             auto y_c0 = libff::alt_bn128_Fq(bls_save_json[idx]["verification_y_c0"].get<std::string>().c_str());
//             auto y_c1 = libff::alt_bn128_Fq(bls_save_json[idx]["verification_y_c1"].get<std::string>().c_str());
//             auto y_coord = libff::alt_bn128_Fq2(y_c0, y_c1);
//             auto z_c0 = libff::alt_bn128_Fq(bls_save_json[idx]["verification_z_c0"].get<std::string>().c_str());
//             auto z_c1 = libff::alt_bn128_Fq(bls_save_json[idx]["verification_z_c1"].get<std::string>().c_str());
//             auto z_coord = libff::alt_bn128_Fq2(z_c0, z_c1);
//             auto g2_vec_0 = libff::alt_bn128_G2(x_coord, y_coord, z_coord);
//             common_public_key = common_public_key + g2_vec_0;
//             for (int32_t poly_idx = 0; poly_idx < local_poly.polynomial_size(); ++poly_idx) {
//                 genesis_nodes[idx]->polynomial.push_back(
//                     libff::alt_bn128_Fr(common::Encode::HexEncode(local_poly.polynomial(poly_idx)).c_str()));
//             }
...
//             genesis_nodes[idx]->verification = dkg_instance.VerificationVector(genesis_nodes[idx]->polynomial);
//             genesis_nodes[idx]->check_hash = check_hash;
...
//             std::cout << "bls_prikey: " << dec_data << std::endl;
//             genesis_nodes[idx]->bls_prikey = libff::alt_bn128_Fr(dec_data.c_str());
//             libBLS::Dkg tmpdkg(valid_t, valid_n);
//             genesis_nodes[idx]->bls_pubkey = tmpdkg.GetPublicKeyFromSecretKey(genesis_nodes[idx]->bls_prikey);
//         }
...
//     auto callback = [&](uint32_t idx, db::DbWriteBatch& dbbatch) -> void {
//         bool file_valid = true;
//         bls::protobuf::LocalPolynomial local_poly;
//         // just private key.
//         genesis_nodes[idx]->polynomial = dkg_instance.GeneratePolynomial();
...
//         bls_save_json[idx]["poly"] = common::Encode::HexEncode(local_poly.SerializeAsString());
//         std::shared_ptr<security::Security> secptr = std::make_shared<security::Ecdsa>();
//         secptr->SetPrivateKey(genesis_nodes[idx]->prikey);
//         genesis_nodes[idx]->verification = dkg_instance.VerificationVector(genesis_nodes[idx]->polynomial);
//         bls_save_json[idx]["verification_x_c0"] = libBLS::ThresholdUtils::fieldElementToString(genesis_nodes[idx]->verification[0].X.c0);
//         bls_save_json[idx]["verification_x_c1"] = libBLS::ThresholdUtils::fieldElementToString(genesis_nodes[idx]->verification[0].X.c1);
//         bls_save_json[idx]["verification_y_c0"] = libBLS::ThresholdUtils::fieldElementToString(genesis_nodes[idx]->verification[0].Y.c0);
//         bls_save_json[idx]["verification_y_c1"] = libBLS::ThresholdUtils::fieldElementToString(genesis_nodes[idx]->verification[0].Y.c1);
//         bls_save_json[idx]["verification_z_c0"] = libBLS::ThresholdUtils::fieldElementToString(genesis_nodes[idx]->verification[0].Z.c0);
//         bls_save_json[idx]["verification_z_c1"] = libBLS::ThresholdUtils::fieldElementToString(genesis_nodes[idx]->verification[0].Z.c1);
//         secret_key_contribution[idx] = dkg_instance.SecretKeyContribution(
//             genesis_nodes[idx]->polynomial, valid_n, valid_t);
//         bls::protobuf::JoinElectInfo join_info;
//         join_info.set_member_idx(idx);
...
//         bls_save_json[idx]["check_hash"] = common::Encode::HexEncode(genesis_nodes[idx]->check_hash);
//         bls_save_json[idx]["verify_g2"] = common::Encode::HexEncode(req->SerializeAsString());
//         bls_save_json[idx]["join_info"] = common::Encode::HexEncode(join_info.SerializeAsString());
//         genesis_nodes[idx]->check_hash = common::Hash::keccak256(str_for_hash);
//         prefix_db_->SaveLocalPolynomial(secptr, secptr->GetAddress(), local_poly, false, dbbatch);
//         prefix_db_->AddBlsVerifyG2(secptr->GetAddress(), *req, dbbatch);
//         prefix_db_->SaveTemporaryKv(genesis_nodes[idx]->check_hash, join_info.SerializeAsString(), dbbatch);
//     };

//     std::vector<std::shared_ptr<std::thread>> thread_vec;
//     for (uint32_t idx = 0; idx < genesis_nodes.size(); ++idx) {
//         // bls_save_json[idx]["id"] = common::Encode::HexEncode(genesis_nodes[idx]->id);
//         // auto thread_callback = [&](uint32_t idx) {
//             db::DbWriteBatch dbbatch;
//             callback(idx, dbbatch);
//             db_->Put(dbbatch);
//         // };
        
//         // thread_vec.push_back(std::make_shared<std::thread>(thread_callback, idx));
...
//     libBLS::Dkg tmpdkg(valid_t, valid_n);
//     auto bls_callback = [&](uint32_t idx) -> void {
//         std::vector<libff::alt_bn128_Fr> tmp_secret_key_contribution;
//         for (uint32_t i = 0; i < genesis_nodes.size(); ++i) {
//             tmp_secret_key_contribution.push_back(secret_key_contribution[idx][i]);
//         }
...
//             genesis_nodes[idx]->id,
//             enc_data);
//         bls_save_json[idx]["bls_prikey"] = common::Encode::HexEncode(enc_data);
//     };

//     for (uint32_t idx = 0; idx < genesis_nodes.size(); ++idx) {
//         bls_callback(idx);
//         // thread_vec.push_back(std::make_shared<std::thread>(bls_callback, idx));
//         // if (thread_vec.size() >= 8 || idx == genesis_nodes.size() - 1) {
//         //     for (uint32_t i = 0; i < thread_vec.size(); ++i) {
//         //         thread_vec[i]->join();
//         //     }
...
//     common_pk_[sharding_id] = common_public_key;
//     FILE* fd = fopen((std::string("/var/") + all_id_hash).c_str(), "w");
//     auto str = bls_save_json.dump();
//     size_t w_size = 0;
//     while (w_size < str.size()) {
//         w_size += fwrite(str.c_str() + w_size, 1, (str.size() - w_size), fd);
//     }
