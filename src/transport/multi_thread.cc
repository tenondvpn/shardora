            ...
            if (etime - btime > 200000lu) {
                for (uint32_t i = 1; i < msg_ptr->times_idx; ++i) {
                    ...
                    if (diff_time > 200000lu) {
                        SHARDORA_INFO("over handle message debug %lu timestamp: %lu, debug: %s, "
                ...
                SHARDORA_INFO("end message handled msg hash: %lu, thread idx: %d, type: %d, use time: %lu, protobuf: %s", 
                    msg_ptr->header.hash64(), thread_idx, msg_ptr->header.type(), (etime - btime),
