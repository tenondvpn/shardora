...
uint32_t TxPool::SyncMissingBlocks(uint64_t now_tm_ms) {
    if (!height_tree_ptr_) {
        ZJC_EMPTY_DEBUG("get invalid height_tree_ptr_ size: %u, latest_height_: %lu", 0, latest_height_);
        return 0;
    }
...
// 
    if (latest_height_ == common::kInvalidUint64) {
        ...
        ZJC_EMPTY_DEBUG("get invalid heights size: %u, latest_height_: %lu", 0, latest_height_);
        return 0;
    }
...
//     prev_synced_time_ms_ = now_tm_ms + kSyncBlockPeriodMs;
    ...
    ZJC_EMPTY_DEBUG("%u get invalid heights size: %u, latest_height_: %lu", 
    ...
    if (invalid_heights.size() > 0) {
        ...
        for (uint64_t i = min_height; i < latest_height_; ++i) {
            if (prefix_db_->BlockExists(net_id, pool_index_, i)) {
                ZJC_EMPTY_DEBUG("block exists now add sync height 1, %u_%u_%lu", 
                    net_id,
                    pool_index_,
                    i);
...
int TxPool::AddTx(TxItemPtr& tx_ptr) {
    ...
    if (tx_ptr->tx_key.empty()) {
        ZJC_EMPTY_DEBUG("add failed unique hash empty: %d", tx_ptr->tx_info->step());
        tx_ptr->tx_key = pools::GetTxMessageHash(*tx_ptr->tx_info);
    }

    if (!IsUserTransaction(tx_ptr->tx_info->step()) && !tx_ptr->tx_info->key().empty()) {
        ZJC_EMPTY_DEBUG("success add system tx step: %d, nonce: %lu, unique hash: %s", 
            tx_ptr->tx_info->step(), 
        ...
        if (over_unique_hash_set_.find(tx_ptr->tx_info->key()) != over_unique_hash_set_.end()) {
            ZJC_EMPTY_DEBUG("trace tx pool: %d, failed add tx %s, key: %s, "
    ...
    ZJC_EMPTY_DEBUG("trace tx pool: %d, success add tx %s, key: %s, nonce: %lu, step: %d", 
        pool_index_,
        common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(), 
        common::Encode::HexEncode(tx_ptr->tx_info->key()).c_str(), 
        tx_ptr->tx_info->nonce(),
...
void TxPool::TxOver(view_block::protobuf::ViewBlockItem& view_block) {
    ...
    for (uint32_t i = 0; i < view_block.block_info().tx_list_size(); ++i) {
        ...
        if (addr.empty()) {
            ZJC_EMPTY_DEBUG("addr is empty: %s", ProtobufToJson(view_block.block_info().tx_list(i)).c_str());
            assert(false);
            continue;
        }
        ...
        auto remove_tx_func = [&](std::map<std::string, std::map<uint64_t, TxItemPtr>>& tx_map) {
            ...
            if (tx_iter != tx_map.end()) {
                for (auto nonce_iter = tx_iter->second.begin(); nonce_iter != tx_iter->second.end(); ) {
                    ZJC_EMPTY_DEBUG("find tx addr success: %s, unique hash: %s, "
                    ...
                    ZJC_EMPTY_DEBUG("find tx addr success: %s, nonce: %lu, remove nonce: %lu", 
                        common::Encode::HexEncode(addr).c_str(), 
                        nonce_iter->first,
                        view_block.block_info().tx_list(i).nonce());
                    if (!IsUserTransaction(view_block.block_info().tx_list(i).step())) {
                        ...
                        ZJC_EMPTY_DEBUG("trace tx pool: %d, success add unique tx %s, key: %s, "
                            "nonce: %lu, step: %d, unique hash exists: %s", 
                            pool_index_,
                    ...
                    ZJC_EMPTY_DEBUG("trace tx pool: %d, over tx addr: %s, nonce: %lu", 
                        pool_index_,
                        common::Encode::HexEncode(addr).c_str(), 
                        nonce_iter->first);
                    auto tx_ptr = nonce_iter->second;
                    ZJC_EMPTY_DEBUG("over pop success add system tx nonce addr: %s, "
                ...
                ZJC_EMPTY_DEBUG("find tx addr failed: %s", common::Encode::HexEncode(addr).c_str());
            }
        };
        ...
        ZJC_EMPTY_DEBUG("trace tx pool: %d, step: %d, to: %s, unique hash: %s, over tx addr: %s, nonce: %lu", 
            pool_index_,
            view_block.block_info().tx_list(i).step(),
            common::Encode::HexEncode(view_block.block_info().tx_list(i).to()).c_str(), 
            common::Encode::HexEncode(view_block.block_info().tx_list(i).unique_hash()).c_str(), 
...
void TxPool::GetTxSyncToLeader(
    ...
    while (added_txs_.pop(&tx_ptr)) {
        ZJC_EMPTY_DEBUG("pop success add system tx nonce addr: %s, addr nonce: %lu, tx nonce: %lu, unique hash: %s",
                common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
                tx_ptr->address_info->nonce(), 
                tx_ptr->tx_info->nonce(),
                common::Encode::HexEncode(tx_ptr->tx_info->key()).c_str());
         if (!IsUserTransaction(tx_ptr->tx_info->step())) {
            ...
            ZJC_EMPTY_DEBUG("success add system tx nonce addr: %s, addr nonce: %lu, tx nonce: %lu, unique hash: %s",
                common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
                tx_ptr->address_info->nonce(), 
                tx_ptr->tx_info->nonce(),
                common::Encode::HexEncode(tx_ptr->tx_info->key()).c_str());
        ...
        if (tx_ptr->address_info->nonce() >= tx_ptr->tx_info->nonce()) {
            ZJC_EMPTY_DEBUG("failed get tx nonce invalid addr: %s, addr nonce: %lu, tx nonce: %lu",
                common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
                tx_ptr->address_info->nonce(), 
                tx_ptr->tx_info->nonce());
            continue;
        ...
        if (iter != consensus_tx_map_.end()) {
            ...
            if (nonce_iter != iter->second.end()) {
                ZJC_EMPTY_DEBUG("exists failed add tx nonce invalid addr: %s, addr nonce: %lu, tx nonce: %lu",
                    common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
                    tx_ptr->address_info->nonce(), 
                    tx_ptr->tx_info->nonce());
                continue;
        ...
        ZJC_EMPTY_DEBUG("success add tx nonce invalid addr: %s, addr nonce: %lu, tx nonce: %lu",
            common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
            tx_ptr->address_info->nonce(), 
            tx_ptr->tx_info->nonce());
    }
        ...
        for (auto nonce_iter = iter->second.begin(); nonce_iter != iter->second.end(); ++nonce_iter) {
            ...
            if (valid_nonce == common::kInvalidUint64) {
                ...
                if (res != 0) {
                    ...
                    ZJC_EMPTY_DEBUG("tx_key invalid: %s",
            ...
            if (!IsUserTransaction(tx_ptr->tx_info->step())) {
                ZJC_EMPTY_DEBUG("nonce invalid: %lu, step is not user tx: %d", 
                    tx_ptr->tx_info->nonce(), 
                    tx_ptr->tx_info->step());
            } else {
                ZJC_EMPTY_DEBUG("trace tx pool: %d, to leader tx addr: %s, nonce: %lu", 
                    pool_index_,
                    common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(), 
                    tx_ptr->tx_info->nonce());
                auto* tx = txs->Add();
...
void TxPool::GetTxIdempotently(
    ...
    TxItemPtr tx_ptr;
    while (added_txs_.pop(&tx_ptr)) {
        ZJC_EMPTY_DEBUG("pop success add system tx nonce addr: %s, "
                "addr nonce: %lu, tx nonce: %lu, unique hash: %s",
                common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
                tx_ptr->address_info->nonce(), 
                tx_ptr->tx_info->nonce(),
        ...
        if (!IsUserTransaction(tx_ptr->tx_info->step())) {
            system_tx_map_[std::to_string(tx_ptr->tx_info->step())][tx_ptr->tx_info->nonce()] = tx_ptr;
            ZJC_EMPTY_DEBUG("success add system tx nonce addr: %s, "
                "addr nonce: %lu, tx nonce: %lu, unique hash: %s",
                common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
                tx_ptr->address_info->nonce(), 
                tx_ptr->tx_info->nonce(),
        ...
        if (iter != consensus_tx_map_.end()) {
            auto nonce_iter = iter->second.find(tx_ptr->tx_info->nonce());
            if (nonce_iter != iter->second.end()) {
                ZJC_EMPTY_DEBUG("exists failed get tx nonce invalid addr: %s, addr nonce: %lu, tx nonce: %lu",
                    common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
                    tx_ptr->address_info->nonce(), 
                    tx_ptr->tx_info->nonce());
                continue;
        ...
        tx_map_[tx_ptr->address_info->addr()][tx_ptr->tx_info->nonce()] = tx_ptr;
        consensus_tx_map_[tx_ptr->address_info->addr()][tx_ptr->tx_info->nonce()] = tx_ptr;
        ZJC_EMPTY_DEBUG("success add tx nonce addr: %s, addr nonce: %lu, tx nonce: %lu",
            common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
            tx_ptr->address_info->nonce(), 
            tx_ptr->tx_info->nonce());
    }
        ...
        for (auto iter = tx_map.begin(); iter != tx_map.end(); ++iter) {
            ...
            for (auto nonce_iter = iter->second.begin(); nonce_iter != iter->second.end(); ++nonce_iter) {
                ...
                if (valid_nonce == common::kInvalidUint64) {
                    ...
                    if (res != 0) {
                        if (res > 0) {
                            ZJC_EMPTY_DEBUG("trace tx pool: %d, tx_key invalid addr: %s, nonce: %lu, unique hash: %s",
                                pool_index_,
                                common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(), 
                                tx_ptr->tx_info->nonce(),
                                common::Encode::HexEncode(tx_ptr->tx_info->key()).c_str());
                ...
                ZJC_EMPTY_DEBUG("trace tx pool: %d, consensus leader tx addr: %s, key: %s, nonce: %lu, "
...
void TxPool::InitLatestInfo() {
    ...
    if (prefix_db_->GetLatestPoolInfo(
        ...
        if (latest_height_ == common::kInvalidUint64 || latest_height_ < pool_info.height()) {
            ...
            ZJC_EMPTY_DEBUG("init latest pool info shard: %u, pool %lu, init height: %lu",
...
uint64_t TxPool::UpdateLatestInfo(
    ...
    if (tmp_height_tree_ptr) {
        ZJC_EMPTY_DEBUG("success set height, net: %u, pool: %u, height: %lu",
    ...
    ZJC_EMPTY_DEBUG("pool index: %d, new height: %lu, new synced height: %lu, prev_synced_height_: %lu, to_sync_max_height_: %lu, latest height: %lu",
...
void TxPool::ConsensusAddTxs(const pools::TxItemPtr& tx_ptr) {
    ...
    ZJC_EMPTY_DEBUG("trace tx pool: %d, sync add tx addr: %s, nonce: %lu", 
        pool_index_,
        common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(), 
        tx_ptr->tx_info->nonce());
