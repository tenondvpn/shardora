...
uint32_t TxPool::SyncMissingBlocks(uint64_t now_tm_ms) {
    if (!height_tree_ptr_) {
        SHARDORA_DEBUG("get invalid height_tree_ptr_ size: %u, latest_height_: %lu", 0, latest_height_);
        return 0;
    }
...
int TxPool::AddTx(TxItemPtr& tx_ptr) {
    ...
    if (!IsUserTransaction(tx_ptr->tx_info->step()) && !tx_ptr->tx_info->key().empty()) {
        ...
        if (over_unique_hash_set_.find(tx_ptr->tx_info->key()) != over_unique_hash_set_.end()) {
            SHARDORA_DEBUG("trace tx pool: %d, failed add tx %s, key: %s, "
...
void TxPool::GetTxIdempotently(
    ...
    SHARDORA_DEBUG("pool: %d, now get tx by leader all: %u, get: %u", pool_index_, all_tx_size(), res_map.size());
}
