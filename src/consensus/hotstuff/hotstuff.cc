...
//     }
    ...
    if (s != Status::kSuccess && !latest_qc_item_ptr_) {
...
void Hotstuff::HandleProposeMsg(const transport::MessagePtr& msg_ptr) {
    ...
    if (pro_msg_wrap->msg_ptr->header.hotstuff().pro_msg().tx_propose().txs_size() == 0) {
        if (!IsEmptyBlockAllowed(*view_block_chain()->HighViewBlock())) {
            return;
        }
    }
...
Status Hotstuff::HandleProposeMsgStep_VerifyQC(std::shared_ptr<ProposeMsgWrapper>& pro_msg_wrap) {
    ...
    ZJC_DEBUG("HandleProposeMsgStep_VerifyQC called hash: %lu, "
