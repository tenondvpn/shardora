...
void Hotstuff::NewView(
    ...
    ZJC_INFO("pool: %d, msg pool: %d, newview, txs size: %lu, view: %lu, "
...
//#ifndef NDEBUG
    ...
    ZJC_INFO("HandleTC called hash: %lu, propose_debug: %s", 
...
void Hotstuff::HandleNewViewMsg(const transport::MessagePtr& msg_ptr) {
    ...
    defer({
            auto e = common::TimeUtils::TimestampMs();
            ZJC_INFO("pool: %d HandleNewViewMsg duration: %lu ms, hash64: %lu",
                pool_idx_, e-b, msg_ptr->header.hash64());
        });

    static uint64_t test_index = 0;
    ++test_index;
    ZJC_INFO("====3.1 pool: %d, newview, message pool: %d, hash64: %lu, test_index: %lu",
        ...
        *tc_ptr.get() = newview_msg.tc();
        ...
        if (tc.view() > pacemaker()->HighTC()->view()) {
            if (!tc.has_view_block_hash()) {
                ...
                ZJC_INFO("newview now verify tc hash: %s, pool index: %u", 
...
//#endif
    ...
    if (!v_block2_info) {
        ZJC_INFO("Failed get v block 2 ref: %s", common::Encode::HexEncode(v_block1->parent_hash()).c_str());
...
//#endif
    ...
    if (!v_block3_info) {
        ZJC_INFO("Failed get v block 3 block hash: %s, %u_%u_%lu", 
...
Status Hotstuff::Commit(
    ...
    while (tmp_block_info != nullptr) {
        ...
        if (!tmp_block_info->valid && !view_block_chain()->view_commited(
            ...
            ZJC_INFO("now commit view block %u_%u_%lu, hash: %s, parent hash: %s", 
                tmp_block->qc().network_id(), 
            ...
            ZJC_INFO("now ignore commit view block %u_%u_%lu, hash: %s, parent hash: %s", 
                tmp_block->qc().network_id(), 
                tmp_block->qc().pool_index(), 
                tmp_block->qc().view(),
                common::Encode::HexEncode(tmp_block->qc().view_block_hash()).c_str(),
...
//#ifndef NDEBUG
    ...
    ZJC_INFO("success commit view block %u_%u_%lu, "
...
void Hotstuff::TryRecoverFromStuck(
    ...
    if (!has_user_tx_tag_ && !has_system_tx) {
        // ZJC_INFO("!has_user_tx_tag_ && !has_system_tx, pool: %u", pool_idx_);
        return;
    }
