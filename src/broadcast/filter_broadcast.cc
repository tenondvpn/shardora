...
void FilterBroadcast::Broadcasting(
    ...
    if (dht_ptr->readonly_hash_sort_dht()->size() < 2u) {
        ZJC_EMPTY_DEBUG("random Broadcasting: %lu, size: %u, dht net: %d",
    ...
    if (message.broadcast().hop_limit() <= message.hop_count()) {
        ZJC_EMPTY_DEBUG("message.broadcast().hop_limit() <= message.hop_count()[%d, %d] hash: %lu",
    ...
    //     // ZJC_EMPTY_DEBUG("layer Broadcasting: %lu, size: %u", msg_ptr->header.hash64(), nodes.size());
    ...
    // } else {
        ...
        ZJC_EMPTY_DEBUG("random Broadcasting: %lu, size: %u",
...
std::vector<dht::NodePtr> FilterBroadcast::GetlayerNodes(
    ...
    for (uint32_t i = 0; i < pos_vec.size(); ++i) {
        if (bloomfilter->find((*hash_order_dht)[pos_vec[i]]->id_hash) != bloomfilter->end()) {
            // ZJC_EMPTY_DEBUG("bloom filtered: %s:%d, %lu, hash64: %lu",
...
//     ZJC_EMPTY_DEBUG("data size: %u, pos_vec size: %u, readobly_dht->size: %u",
//         bloomfilter->data().size(), pos_vec.size(), readobly_dht->size());
//     for (uint32_t i = 0; i < bloomfilter->data().size(); ++i) {
//         ZJC_EMPTY_DEBUG("data i: %d, data: %lu", i, bloomfilter->data()[i]);
//     }
...
void FilterBroadcast::Send(
    ...
    for (uint32_t i = 0; i < nodes.size(); ++i) {
        int res = transport::TcpTransport::Instance()->Send(
            ...
            nodes[i]->public_port,
            msg_ptr->header);
        ZJC_EMPTY_DEBUG("broadcast random send to: %s:%d, txhash: %lu, res: %u",
...
void FilterBroadcast::LayerSend(
    ...
    for (uint32_t i = 0; i < nodes.size(); ++i) {
        ...
        ZJC_EMPTY_DEBUG("broadcast layer send to: %s:%d, txhash: %lu",
