...
public:
    ...
    pools::TxItemPtr GetStatisticTx(uint32_t pool_index, bool leader, uint64_t tm_height);
...
//     std::shared_ptr<pools::protobuf::ToTxHeights> to_tx_heights_ptr_ = nullptr;
    ...
    std::shared_ptr<std::map<uint64_t, std::shared_ptr<BlockTxsItem>>> shard_statistics_map_ptr_ = nullptr;
