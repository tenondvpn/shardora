...
public:
    ...
    std::shared_ptr<block::protobuf::Block> CreateElectBlock(
        ...
        auto shardora_block = std::make_shared<block::protobuf::Block>();
        auto tx_list = shardora_block->mutable_tx_list();
        ...
        shardora_block->set_prehash(root_pre_hash);
        shardora_block->set_version(common::kTransactionVersion);
        shardora_block->set_pool_index(common::kRootChainPoolIndex);
        shardora_block->set_height(height);
        ...
        for (uint32_t i = 0; i < bitmap_data.size(); ++i) {
            shardora_block->add_bitmap(bitmap_data[i]);
        }

        shardora_block->set_network_id(2);
        shardora_block->set_hash(block::GetBlockHash(*shardora_block));
        return shardora_block;
    }
};
