...
TEST_F(TestBls, AllSuccess) {
    ...
    std::vector<libff::alt_bn128_G1> all_signs;
    bls::protobuf::VerifyVecBrdReq proto_signs;
    for (size_t i = 0; i < pri_vec.size(); ++i) {
        BlsSign bls_sign;
        libff::alt_bn128_G1 sign;
    ...
    for (int32_t i = 0; i < (n - t - 1); ++i) {
        ...
        for (int32_t j = i; j < (i + t); ++j) {
            idx_vec[j - i] = j + 1;
            tmp_all_signs.push_back(all_signs[j]);
        }
        ...
        for (uint32_t count = 0; count < n; ++count) {
            ...
            // slow
            ASSERT_EQ(
                bls_sign.Verify(t, n, agg_sign, hash, dkg[count].common_public_key_, &verify_hash),
                kBlsSuccess);
        }
    }
}
