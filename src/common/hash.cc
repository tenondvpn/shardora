...
std::string Hash::Sha256(const std::string& str) {
    SHA256_CTX context{};
    SHA256_Init(&context);
    SHA256_Update(&context, (unsigned char*)(str.c_str()), str.size());
...
(c) = ROL((c), 10);\
    ...
    void MDfinish(uint32_t *MDbuf, uint8_t const *strptr, uint32_t lswlen, uint32_t mswlen)
    {
...
std::string Hash::ripemd160(const std::string& str) {
    uint8_t hashcode[20];
    uint32_t buffer[RMDsize / 32];
    uint32_t current[16];

    rmd160::MDinit(buffer);
    uint8_t const* message = (uint8_t*)str.c_str();
    uint32_t remaining = str.size();
    ...
    rmd160::MDfinish(buffer, message, str.size(), 0);

    for (unsigned i = 0; i < RMDsize / 8; i += 4)
    {
        hashcode[i] = buffer[i >> 2];
        hashcode[i + 1] = (buffer[i >> 2] >> 8);
        hashcode[i + 2] = (buffer[i >> 2] >> 16);
        hashcode[i + 3] = (buffer[i >> 2] >> 24);
    }

    return std::string((char*)hashcode, sizeof(hashcode));
}

std::string Hash::keccak256(const std::string& str) {
    auto h = ethash::keccak256((uint8_t*)str.c_str(), str.size());
    return std::string((char*)h.bytes, sizeof(h.bytes));
}
