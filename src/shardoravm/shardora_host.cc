...
void ShardorahainHost::emit_log(const evmc::address& addr,
    ...
    if (topics_count >= 3 && topics[0] == kCrossStorageOutTopic) {
        ...
        if (data_size >= 160) {
            ...
            SHARDORA_DEBUG("CrossStorageOut intercepted from %s shard=%u pool=%u, cross_gas_charged=%ld",
    ...
    if (topics_count >= 1 && topics[0] == kCrossTransferOutTopic) {
        ...
        if (data_size >= 160 && topics_count >= 3) {
            ...
            action.amount_bytes = std::string(reinterpret_cast<const char*>(data + 32), 32);
            action.amount = 0;
            for (int i = 0; i < 8; ++i) {
