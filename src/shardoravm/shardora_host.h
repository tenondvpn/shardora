...
public:
    ...
    void MergeToPrev() {
        ...
        for (auto iter = accounts_.begin(); iter != accounts_.end(); ++iter) {
            auto old_iter = pre_shardora_host_->accounts_.find(iter->first);
            if (old_iter == pre_shardora_host_->accounts_.end()) {
                pre_shardora_host_->accounts_[iter->first] = iter->second;
                continue;
            }
        ...
        for (auto iter = cross_to_map_.begin(); iter != cross_to_map_.end(); ++iter) {
            pre_shardora_host_->cross_to_map_[iter->first] = iter->second;
        }
