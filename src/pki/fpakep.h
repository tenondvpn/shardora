...
class Fpakep {
public:
    // String concatenation function using append method
    ...
    void sm4_encrypt_h(const std::vector<uint8_t>& plaintext, const std::vector<uint8_t>& key, std::vector<uint8_t>& ciphertext) {
        ...
        size_t block_size = 16; // SM4 block size is 16 bytes
        std::vector<uint8_t> padded_plaintext = pkcs7_pad(plaintext, block_size); // Pad plaintext

        size_t num_blocks = padded_plaintext.size() / block_size;

        ciphertext.resize(num_blocks * block_size, 0); // Initialize ciphertext to zero
    ...
    //     plaintext.resize(ciphertext.size()); // Initialize plaintext to zero

    //     for (size_t i = 0; i < num_blocks; ++i) {
    //         sm4_decrypt(&sm4_key, ciphertext.data() + i * block_size, plaintext.data() + i * block_size);
    //     }

    //     plaintext = pkcs7_unpad(plaintext); // Remove padding
    // }
    ...
    void print_hash(const uint8_t hash[32]) {
        ...
        std::cout << std::dec << std::endl; // Restore to decimal output
    }

    // Generate SM2 key pair, allow manual setting of private key and generate public key from private key
    ...
    std::vector<uint8_t> fp_sm2_encrypt(const SM2_KEY& key, const std::vector<uint8_t>& plaintext) {
        std::vector<uint8_t> ciphertext(plaintext.size() + 128, 0); // Maximum overhead
    ...
    int InitPrivateAndPublicKey(const std::string& init_str, const std::string& des_file) {
        ...
        // plaintext_str_X1 as the ciphertext X1 of the other party
