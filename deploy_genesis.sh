...
cd /root && sh -x fetch.sh 127.0.0.1 ${server0} '' '/root' r1 s3_2 s4_2
echo "==== 同步中继服务器 ====" 
...
cd /root && sh -x fetch.sh ${server0} ${server1} '' '/root' s3_1 s4_1;
...
rm -rf /root/zjnodes;
sshpass -p '' scp -o StrictHostKeyChecking=no root@"${server0}":/root/fetch.sh /root/
cd /root && sh -x fetch.sh ${server0} ${server2} '' '/root' r2 s3_3 s4_3;
...
rm -rf /root/zjnodes;
sshpass -p '' scp -o StrictHostKeyChecking=no root@"${server0}":/root/fetch.sh /root/
cd /root && sh -x fetch.sh ${server0} ${server3} '' '/root' r3 s3_4 s4_4;
