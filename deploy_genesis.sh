...
cd /root/xf && sh -x fetch.sh 127.0.0.1 ${server0} '!@#$%^' '/root/xf' r1 r2 s3_1 s3_2 s3_3 s4_1 s4_2 s4_3 s4_4
# EOF
...
cd /root/xf && sh -x fetch.sh ${server0} ${server1} '!@#$%^' '/root/xf' s3_6 s4_7
EOF
) &
...
rm -rf /root/xf/zjnodes;
sshpass -p '!@#$%^' scp -o StrictHostKeyChecking=no root@"${server0}":/root/xf/fetch.sh /root/xf/
cd /root/xf && sh -x fetch.sh ${server0} ${server2} '!@#$%^' '/root/xf' r3 s3_4 s3_5 s4_5 s4_6
EOF
) &
