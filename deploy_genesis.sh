...
cd /root && sh -x fetch.sh 127.0.0.1 ${server0} 'Xf4aGbTaf!' '/root' r1 r3 s3_2 s3_4 s3_6 s3_8 s3_10 s3_12 s3_14 s3_16 s3_18 s3_20
echo "==== 同步中继服务器 ====" 
...
cd /root && sh -x fetch.sh ${server0} ${server1} 'Xf4aGbTaf!' '/root' r2 s3_1 s3_3 s3_5 s3_7 s3_9 s3_11 s3_13 s3_15 s3_17 s3_19;
...
(
echo "[$server0]"
for n in r1 r3 s3_2 s3_4 s3_6 s3_8 s3_10 s3_12 s3_14 s3_16 s3_18 s3_20; do
...
echo "[$server1]"
sshpass -p 'Xf4aGbTaf!' ssh -o StrictHostKeyChecking=no root@$server1 <<EOF
for n in r2 s3_1 s3_3 s3_5 s3_7 s3_9 s3_11 s3_13 s3_15 s3_17 s3_19; do
    ln -s /root/zjnodes/zjchain/GeoLite2-City.mmdb /root/zjnodes/\${n}/conf
    ln -s /root/zjnodes/zjchain/conf/log4cpp.properties /root/zjnodes/\${n}/conf
    ln -s /root/zjnodes/zjchain/zjchain /root/zjnodes/\${n}
done
...
for n in s3_1 s3_3 s3_5 s3_7 s3_9 s3_11 s3_13 s3_15 s3_17 s3_19; do
...
for n in s3_2 s3_4 s3_6 s3_8 s3_10 s3_12 s3_14 s3_16 s3_18 s3_20; do
    cp -rf /root/zjnodes/zjchain/shard_db_3 /root/zjnodes/${n}/db
done
) &
wait
...
for node in r2 s3_1 s3_3 s3_5 s3_7 s3_9 s3_11 s3_13 s3_15 s3_17 s3_19; do \
...
echo "[$server0]"
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/gcc-8.3.0/lib64
for node in r3 s3_2 s3_4 s3_6 s3_8 s3_10 s3_12 s3_14 s3_16 s3_18 s3_20; do
cd /root/zjnodes/$node/ && nohup ./zjchain -f 0 -g 0 $node root> /dev/null 2>&1 &
done
