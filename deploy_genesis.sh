...
cd /root && sh -x fetch.sh ${server0} ${server1} 'Xf4aGbTaf!' '/root' r3 s3_4 s3_8 s3_12 s3_16 s3_20;
...
rm -rf /root/zjnodes;
sshpass -p 'Xf4aGbTaf!' scp -o StrictHostKeyChecking=no root@"${server0}":/root/fetch.sh /root/
cd /root && sh -x fetch.sh ${server0} ${server2} 'Xf4aGbTaf!' '/root' r2 s3_3 s3_7 s3_11 s3_15 s3_19;
