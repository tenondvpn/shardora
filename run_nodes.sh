...
server3=10.101.20.31
server4=10.101.20.30
server5=10.101.20.32
...
cd /root && sh -x fetch.sh 127.0.0.1 ${server0} $pass r1 s3_2 s3_9 s3_16 s3_23 s3_30 s4_2 s4_9 s4_16 s4_23 s4_30
EOF
...
cd /root && sh -x fetch.sh ${server0} ${server1} !@#$%^ r6 s3_7 s3_14 s3_21 s3_28 s3_35 s4_7 s4_14 s4_21 s4_28 s4_35
EOF
...
sshpass -p !@#$%^ scp root@"${server0}":/root/fetch.sh /root/
cd /root && sh -x fetch.sh ${server0} ${server2} !@#$%^ r2 s3_3 s3_10 s3_17 s3_24 s3_31 s4_3 s4_10 s4_17 s4_24 s4_31
EOF
...
sshpass -p !@#$%^ scp root@"${server0}":/root/fetch.sh /root/
cd /root && sh -x fetch.sh ${server0} ${server3} !@#$%^ r4 s3_5 s3_12 s3_19 s3_26 s3_33 s4_5 s4_12 s4_19 s4_26 s4_33
EOF
...
sshpass -p !@#$%^ scp root@"${server0}":/root/fetch.sh /root/
cd /root && sh -x fetch.sh ${server0} ${server4} !@#$%^ r5 s3_6 s3_13 s3_20 s3_27 s3_34 s4_6 s4_13 s4_20 s4_27 s4_34
EOF
...
sshpass -p !@#$%^ scp root@"${server0}":/root/fetch.sh /root/
cd /root && sh -x fetch.sh ${server0} ${server5} !@#$%^ r3 s3_4 s3_11 s3_18 s3_25 s3_32 s4_4 s4_11 s4_18 s4_25 s4_32
EOF
...
export LD_LIBRARY_PATH=\$LD_LIBRARY_PATH:/usr/local/gcc-8.3.0/lib64; \
for node in r6 s3_7 s3_14 s3_21 s3_28 s3_35 s4_7 s4_14 s4_21 s4_28 s4_35; do \
    cd /root/zjnodes/\$node/ && nohup ./zjchain -f 0 -g 0 \$node > /dev/null 2>&1 &\
done \
'"
...
export LD_LIBRARY_PATH=\$LD_LIBRARY_PATH:/usr/local/gcc-8.3.0/lib64; \
for node in r4 s3_5 s3_12 s3_19 s3_26 s3_33 s4_5 s4_12 s4_19 s4_26 s4_33; do \
    cd /root/zjnodes/\$node/ && nohup ./zjchain -f 0 -g 0 \$node > /dev/null 2>&1 &\
done \
'"
...
export LD_LIBRARY_PATH=\$LD_LIBRARY_PATH:/usr/local/gcc-8.3.0/lib64; \
for node in r5 s3_6 s3_13 s3_20 s3_27 s3_34 s4_6 s4_13 s4_20 s4_27 s4_34; do \
    cd /root/zjnodes/\$node/ && nohup ./zjchain -f 0 -g 0 \$node > /dev/null 2>&1 &\
done \
'"
