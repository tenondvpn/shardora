 ...
 *              field GF(2^6m) - uses irreducible polynomial x^6+x^FA+x^FB+x^FC+1
 *
...
public:
    GF2m6x()                    { }
    GF2m6x(const GF2m6x & b)    {for (int i=0;i<FM;i++) x[i]=b.x[i]; }
    GF2m6x(int i)               {x[0]=i; }
    GF2m6x(const GF2m* a)       {for (int i=0;i<FM;i++) x[i]=a[i];}
    GF2m6x(const GF2m& a,const GF2m& b, const GF2m& c=0, const GF2m& d=0, const GF2m& e=0, const GF2m& f=0)
                                {x[0]=a; x[1]=b; x[2]=c; x[3]=d; x[4]=e; x[5]=f;} 

    GF2m6x(const Big& a) {x[0]=(GF2m)a;}
    ...
    GF2m6x& operator=(const GF2m6x& b)
        { for (int i=0;i<FM;i++) x[i]=b.x[i]; return *this; }
    GF2m6x& operator=(const GF2m& b)
        { x[0]=b; for (int i=1;i<FM;i++) x[i]=0; return *this; }
    GF2m6x& operator=(int b)
        { x[0]=b; for (int i=1;i<FM;i++) x[i]=0; return *this; }
    GF2m6x& operator+=(const GF2m6x& b) 
        { for (int i=0;i<FM;i++) x[i]+=b.x[i]; return *this; }
    GF2m6x& operator+=(const GF2m& b)
        {x[0]+=b; return *this; }
    GF2m6x& operator*=(const GF2m6x&);
    GF2m6x& operator*=(const GF2m&);
    GF2m6x& operator/=(const GF2m6x&);
    GF2m6x& operator/=(const GF2m&);
