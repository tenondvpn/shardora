...
namespace SimpleWeb {
  ...
  public:
    ...
    public:
      ...
      std::size_t max_request_streambuf_size = 1048576;
  ...
  private:
    ...
    public:
      regex_orderable(const char *regex_cstr) : str(regex_cstr) {}
      regex_orderable(std::string regex_str_) : str(std::move(regex_str_)) {}
      bool operator<(const regex_orderable &rhs) const noexcept {
        return str < rhs.str;
      }
    };
    ...
    void find_resource(const std::shared_ptr<Session> &session) {
      ...
      for(auto &regex_method : resource) {
        ...
        if(it != regex_method.second.end()) {
          ...
          if(regex_method.first.str == session->request->path) {
