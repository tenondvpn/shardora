...
| **M6 子分片广播** | **核心创新（使能性）** | 委员会从候选池随机选 $R$ 个中继节点，各向其 $J=10$ 个子分片注入块；复用现有 P2P 子分片拓扑，委员会发送量从 $O(N)$ 降至 $O(R)$，零新增基础设施，无 BLS 聚合逻辑 |
...
| **子分片 P2P 拓扑**（每节点加入 J=10 子分片） | 已有实现（确定性分配 + 子分片内 Gossip） |
| **JoinElectTx 容量控制**（每子分片 ≤ 1024 节点） | root shard 共识强制，已有实现 |
...
| **M6 中继（被选中时）** | 待实现 | 被选为 relay_set 成员时，将块广播到所属 J=10 个子分片（走现有块同步路径） |
...
| M6 广播延迟测试 | **实机**（$R=200$ 中继，$P=977$ 子分片） | 验证 2 跳传播延迟 ≤ $t_b/2=5$s；测量 pull 同步触发频率 |
...
2. **块广播效率**：直接 Gossip vs. M6 随机中继（$R=200$，$J=10$）的委员会发送量对比（目标：$O(N)$ → $O(R)=200$ 条/块）
