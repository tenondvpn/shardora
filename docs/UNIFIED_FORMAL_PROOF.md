...
>
> **核心结论**：在 A1–A10 条件下，方案达到 BFT 安全性（$\approx2^{-72}$，$k=1024$，$\beta\leq0.2$，定理 4.1）、有效 Nakamoto 系数理论上界的 $95.4\%$（31,790）、线性通信复杂度最优（$O(k)$），且 TPS 与候选池规模完全解耦。
>
...
| 安全性 S | $\Pr[\text{攻破}]\leq2^{-72}$（$k=1024$，$\beta\leq0.2$，定理 4.1） | A1–A9 全部满足 | $1-\mathrm{negl}(\lambda)$（BFT 下界） |
