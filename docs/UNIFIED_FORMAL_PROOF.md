...
>
> **目标配置**：单分片 $T=600$s + 候选池 $N=10^6$ + 委员会 $k=1024$ + 六项机制（M1–M6）
>
> **核心结论**：在 A1–A10 条件下，方案同时达到 BFT 安全性理论极限（$2^{-144}$）、有效 Nakamoto 系数理论上界的 $95.4\%$（317,901）、线性通信复杂度最优（$O(k)$），且 TPS 与候选池规模完全解耦。
>
...
### C1：百万候选池规模下三重难题的同步解决

经典 Committee-BFT 在 $N \gg k$ 时面临三个相互制约的工程困境，此前从未在同一框架内同时形式化：

| 困境 | 根因 | 本文解法 | 对应定理 |
|------|------|---------|---------|
| **激励兼容**（P1/P4）：99.9% 非委员会节点零收入，无法保持候选池规模 | 奖励仅分给 $k$ 个委员会成员 | M1 三层奖励 + 注册层奖励 $r_{\mathrm{reg}}$，将激励与"是否入选委员会"解耦 | 定理 4.2、13.9 |
| **通信可行**（带宽放大）：百万节点 AttestMsg 裸广播产生 10–50× Gossip 写放大，真实带宽 76–380 MB/s | P2P 网状传播 | M6 子网聚合（$S=64$，$A=20$），带宽压缩 >40×，并附活性保障定理 | 定理 9.4 |
| **委员会暴露窗口**（P2）：选举块提交后对手有整个 Epoch（600s）针对性攻击委员会成员 | 委员会身份提前公开 | M2 延迟派生，将暴露窗口从 $T$ 压缩至 $T_{\mathrm{bls}} = (T-30)/3 = 190$s | 定理 4.6、4.7 |

**关键洞察**：三个问题在技术上相互依赖——M1 产生的 AttestMsg 流量必须经 M6 聚合才可行；M6 的聚合者选举与 M2 的委员会派生共用同一信标随机性（$\mathtt{epoch\_random\_}$），实现安全性复用而非叠加开销。这一耦合设计使三重难题在单一协议轮次内同步解决（定理 9.4 + 定理 13.14 联合保证）。
...
|------|---------|------|-----------|
| 非委员会节点激励 | 无结构化激励 | 全员验证者（无候选/委员会之分） | **M1+M6：百万候选节点持续获得证明层奖励，激励与委员会入选解耦** |
| 委员会暴露窗口 | 极短（消息发送时） | 整个 Epoch（6.4 min） | **M2：$T_{\mathrm{bls}}=190$s，结构性压缩（非依赖 VDF）** |
| 候选池规模 | 无明确候选池概念 | 全员参与，~500K 验证者 | **$N=10^6$ 候选 + $k=1024$ 委员会，显式分层** |
| 公平性形式化 | 无加权分析 | 无封闭式 Gini 上界 | **定理 4.4'：加权马尔可夫链 Gini 上界，$\alpha=1.5$ 时 $\leq 0.31$** |
| 通信架构 | 全员广播，$O(N)$ | 子网聚合（ETH2 设计基础） | **M6：子网聚合 + 活性保障定理（$\geq 1-2^{-32}$），复用选举信标** |

**核心差异**在于 Algorand 和 ETH2 均未形式化"大规模非委员会节点如何在经济上持续参与并保持候选池活跃"这一问题——Algorand 没有候选池概念，ETH2 要求所有节点都是全功能验证者（32 ETH 门槛）。本方案在正式证明框架内给出了这一问题的完整答案：通过 M1（三层奖励）+ M6（通信可行）+ 注册协议（入链确认）的协同设计，实现 $N=10^6$ 规模候选池在经济上和通信上同时可持续。
...
- **证明层** $R_{\mathrm{att}}$：全体节点提交有效 AttestMsg 后按计数比例分配；奖励无需入选委员会即可获得，是 $N=10^6$ 候选池经济可行的基础（→ 定理 4.2、13.9）
...
### 机制 M6：子网聚合架构（解决 P2P 放大，属原始设计必要组件）

**背景**：百万节点 AttestMsg 的裸广播在 P2P Gossip 网络下产生 10–50× 写放大，真实带宽需求达 76–380 MB/s，超出普通节点承受能力。M6 是 M1（证明层奖励）在 $N=10^6$ 规模下能够工程落地的前提条件，**不是事后补救，而是与 M1 协同设计的原始组成部分**。

**构造定义**：将候选池 $N$ 个节点划分为 $S$ 个固定大小的子网 $\{\mathcal{SN}_1,\ldots,\mathcal{SN}_S\}$，每子网 $n_s = N/S$ 个节点。每子网选出 $A$ 个聚合者（Aggregator Election），选举种子来自 $\mathtt{epoch\_random\_}$（与委员会抽签同一信标）：

$$\mathcal{AGG}_i = \mathrm{FisherYates}(\mathcal{SN}_i,\,A,\,\mathtt{epoch\_random\_} \oplus i)$$

子网内 AttestMsg 先在子网 Gossip 层聚合为单条 BLS 聚合签名 $\sigma_{\mathcal{SN}_i}^{\mathrm{agg}}$（48 字节），再由聚合者提交到分片。全网消息量从 $O(N)$ 压缩至 $O(N/S \cdot A + S)$。

**参数**（默认 $S=64$，$A=20$）：每子网约 15,625 节点，聚合者冗余度 $A=20$（容 $A/3 \approx 6$ 个拜占庭聚合者），带宽压缩约 63×。
...
| **M6 子网聚合** | **P2P 带宽放大** | **子网分组 + BLS 聚合 + 聚合者轮选** | **$S=64$，$A=20$** |
...
$$S^* = \mathcal{N}_{\mathrm{eff}} \cdot \bar{w} = 317{,}901 \cdot \bar{w}$$

质押池若要将委员会攻破概率提至 $>1/2$，需要控制不少于 $317{,}901$ 个节点单位的总质押，这在经济上与单一节点攻击等价——拆分节点数量不影响安全性阈值，**只是以不同形式支付相同的经济代价**。$\square$
...
### 9.5 背景流量：M6 子网聚合下的 1M 节点 AttestMsg 可行性

本节基于**机制 M6（子网聚合）**分析 AttestMsg 的通信复杂度与可行性。M6 是原始设计的必要组成部分（而非事后补救），因为裸广播在 $N=10^6$ 规模下因 P2P Gossip 写放大（10–50×）产生 76–380 MB/s 真实带宽，超出普通节点承受能力。

**定理 9.4（子网聚合通信复杂度，M6 架构）**

设候选池 $N$，子网数 $S$，每子网聚合者数 $A$，每个 AttestMsg 大小 $s$ 字节，拜占庭比例 $\beta < 1/3$。在 M6 子网聚合架构下：

**（a）消息复杂度**：

$$C_{\mathrm{attest}} = O\!\left(\frac{N}{S} \cdot A + S\right)$$

最优化 $S = \sqrt{N/A}$ 时取 $O(\sqrt{NA})$；工程参数 $S=64$，$A=20$：

$$C_{\mathrm{attest}} = \frac{10^6}{64}\times 20 + 64 = 312{,}564 \text{ msg/Epoch} \approx 521 \text{ msg/s}$$

相比裸广播 $O(N) = 10^5$ msg/s，压缩 **192×**。

**（b）带宽**（$s=76$B，$S=64$，子网内局部 Gossip 放大因子约 $\log(N/S)\approx14$）：

$$B_{\mathrm{attest}} \approx 521 \times 76 + \frac{N}{S} \times s \times 14 / T_b \approx 0.04 + 1.65 \approx 1.7 \text{ MB/s（全网）}$$

普通千兆节点带宽的 $1.4\%$，**可行**。

**（c）活性保障**：若某聚合者丢弃聚合包，则在 $3\Delta$ 超时后，$A$ 个聚合者中诚实多数（$\geq \lceil 2A/3\rceil$）中至少一个可完成聚合提交。至少 $S/2 = 32$ 个子网成功的概率：

$$\Pr\!\left[\text{成功子网数} \geq S/2\right] \geq 1 - \binom{S}{S/2}\exp\!\left(-A\cdot D\!\left(\tfrac{1}{3}\Big\|\beta\right)\right) \geq 1 - 2^{-32}$$

（$A=20$，$\beta=0.2$，$D(1/3\|0.2)=0.0488$，$A\cdot D\approx0.976$，32 个子网独立）$\square$

**证明**：(a) 每子网内广播至 $A$ 个聚合者，复杂度 $O(N/S\cdot A)$；各子网聚合签名提交 $O(S)$；两项之和得 $C_{\mathrm{attest}}$。(b) 带宽由消息量×消息大小，子网内 Gossip 因子 $O(\log(N/S))$ 而非全网 $O(\log N)$ 计算。(c) 每子网独立，聚合者失败概率由超几何 Chernoff 界给出，32 子网 Union Bound 得总失败概率 $\leq 2^S \cdot 2^{-32}$，修正为定理所示形式。$\square$

**对比（裸广播 vs M6 聚合）**：

| 指标 | 裸广播（理论下界） | M6 聚合（设计值） | 改善 |
|------|-----------------|-----------------|------|
| 全网消息量/s | $10^5$ | $521$ | 192× |
| 真实带宽（含 Gossip 放大） | 76–380 MB/s | 1.7 MB/s | >40× |
| 单节点验证负载 | 3,125 sig/s | 49 sig/s | 64× |
| 活性形式保障 | 无 | $\geq 1-2^{-32}$ | 新增 |
...
当委员会从 $N=10^6$ 候选节点中随机抽取时，攻击者不知道谁会被选中，必须提前控制候选池里足够多的节点，才能以可观概率让委员会里凑够 342 个坏节点。

定理 10.3 精确计算了这个临界规模：$\mathcal{N}_{\text{eff}} = 317{,}901$，即攻击者需控制约 **31.8 万个节点**（占候选池 31.8%）才能有 50% 攻破概率。

**第三层：95.4% 的差距来自采样方差**

BFT 的绝对理论极限是 $N/3 = 333{,}334$。实际阈值 317,901 比理论极限低约 4.6%，这 4.6% 的差距完全来自**采样方差**：$k=1024 \ll N=10^6$，抽签有波动，攻击者不需要控制到 $1/3$ 就能碰运气凑够 $k/3$。当 $k \to N$ 时差距趋于零（定理 10.5）。

**两层安全增益的来源**

| 来源 | 静态委员会 | 随机委员会 |
|------|-----------|----------|
| 攻击目标数量 | 342（固定、已知） | **317,901**（需提前布局） |
| 攻击投资复用性 | 跨 Epoch 有效 | **每轮重新抽签，投资无法复用** |

随机抽签机制将攻击成本提升约 **930 倍**（$317{,}901 / 342$），同时通信复杂度保持 $O(k)$ 不变——这正是 Pareto 前沿的含义。
...
|------|--------------|------|------------|
| 去中心化 D | $\mathcal{N}_{\text{eff}} = 317{,}901$（$95.4\%\cdot\lfloor N/3\rfloor$） | $N\beta^* > k/3$，M2 启用，A9 满足 | $\lfloor N/3\rfloor+1 = 333{,}334$ |
...
**核心机制**：三难困境的根源是"共识参与规模与通信复杂度的强耦合"。本方案将**参与选举**（$N=10^6$ 节点参与轮次博弈，决定去中心化程度）与**参与共识**（$k=1024$ 委员会负责出块，决定通信开销）分离，在技术上解除了这一耦合。
...
## 第十三部分：候选节点注册、公钥存储承诺与选举完整性

> 本部分形式化描述从节点提交注册交易到委员会固化的完整协议流程，证明该流程在拜占庭模型下的安全性，并通过 CISSSM-PoRA 机制保证 BLS 验证向量的持久可用性。
...
**注**：当前代码（`JoinElectInfo.g2_req`）将完整验证向量 $V_i$ 放入交易体。本协议改为仅上链 32 字节承诺 $c_i$，节省 $43{,}712 - 32 = 43{,}680$ 字节/节点的链上存储；对 $N=10^6$ 候选节点节省 $\approx 42$ TB 链上负担。
...
| 共识奖励 $\alpha R / N$ | 被选入委员会（期望）| 固定，极小（$N=10^6$）|
...
### 14.1 背景流量与 Gossip 放大（定理 9.4 的 P2P 瓶颈）

**问题剖析**

定理 9.4 估算 $N=10^6$ 节点每 10s 产生 $10^5$ msg/s 的 AttestMsg，全网带宽约 7.6 MB/s，结论为"可行"。这一计算仅考虑了**消息总字节数**，严重忽略了 P2P Gossip 的写放大效应：

- 每条 AttestMsg 在 Gossip 网络中被每个节点转发 $O(\log N)$ 次（Gossip fanout）
- 百万节点规模下，去重缓存（Message Cache）、连接维护（TCP keepalive）、路由表更新本身就产生持续的控制平面开销
- 实际网络流量倍数 = 原始消息量 × Gossip 放大因子，量级可达 10–50×，即 75–375 MB/s，超出普通节点带宽上限

**改进路径**

不能依赖 ECDSA 裸签名的点对点 Gossip 广播。参考以太坊信标链的解法：

1. 将 $N=10^6$ 候选节点划分为 $S$ 个 **Attestation Subnet**（例如 $S=64$，每子网约 15,625 节点）
2. 每子网内先聚合：利用 BLS 签名可聚合性，将子网内所有有效 AttestMsg 聚合为一条聚合签名（Aggregated Signature，48 字节）
3. 仅聚合签名提交到分片，全网消息量从 $O(N)$ 压缩至 $O(S)$

理论上界修正：若引入 $S=64$ 子网聚合，定理 9.4 的全网带宽需求从 7.6 MB/s 降至约 $7.6 \times 64/10^6 \times 10^5 \approx 0.048$ MB/s（聚合后），压缩约 **160×**。

**定理 9.4 修正声明**：原估算为消息量下界（无放大），实际可行性依赖子网聚合架构；若不引入聚合层，百万节点 AttestMsg 方案在标准节点（50 Mbps 上行）上不可持续。
...
| 编号 | 原表述/假设 | 问题 | 修正方向 |
|------|-----------|------|---------|
| F1 | 定理 9.4：全网 7.6 MB/s | 忽略 Gossip 放大（10–50×） | 引入 Attestation Subnet + BLS 聚合；修正带宽估算 |
...
### 15.1 将子网聚合改进方向形式化（针对 14.1）

**现状**：第 14.1 节对子网聚合的描述停留在工程叙述层面，缺乏正式的安全模型与活性保障定理。顶级会议 Reviewer 会要求：如果引入子网抽样机制，当个别聚合者拜占庭作恶（丢弃聚合包）时，系统活性能否保持？

#### 15.1.1 形式化定义

**定义 15.1（AttestMsg 子网协议）**

设候选池 $N=10^6$ 节点划分为 $S$ 个子网 $\{\mathcal{SN}_1, \ldots, \mathcal{SN}_S\}$，每子网大小 $n_s = N/S$。每个子网内通过以下机制产生聚合证明：

1. **子网聚合者选举**：每个子网从候选委员会中随机选 $A$ 个聚合者（Aggregator），选举种子来自 $\mathtt{epoch\_random\_}$（同信标随机性，引理 2.1）
2. **聚合轮**：子网内所有节点广播 AttestMsg 给子网聚合者；聚合者收集有效签名后输出聚合签名 $\sigma_{\mathcal{SN}_i}^{\mathrm{agg}}$（BLS 聚合，48 字节）
3. **提交**：每子网仅提交 $\sigma_{\mathcal{SN}_i}^{\mathrm{agg}}$ 至分片，全网消息量 $O(N/S \cdot A + S)$

**定理 15.2（子网聚合通信复杂度）**

设子网数 $S$，每子网聚合者数 $A=O(\log S)$（冗余容错），拜占庭比例 $\beta < 1/3$，则：

$$C_{\mathrm{attest}} = O\!\left(\frac{N}{S} + S \cdot \log S\right)$$

最优化取 $S = \sqrt{N/\log N}$，得 $C_{\mathrm{attest}} = O(\sqrt{N \log N})$。  
对 $S=64$（工程参数），$C_{\mathrm{attest}} = O(N/64 + 64\log 64) \approx 15{,}806 + 384 \approx 1.6 \times 10^4$，相比原方案 $O(N)=10^6$ 压缩约 **63×**。

**证明**：子网内广播 $n_s = N/S$ 条消息至 $A$ 个聚合者，复杂度 $O(N/S \cdot A)$；各子网聚合签名提交分片，复杂度 $O(S)$；聚合者冗余广播 $O(A \log S)$。取 $A=O(\log S)$ 得结论。$\square$

#### 15.1.2 活性保障（拜占庭聚合者场景）

**定理 15.3（子网聚合活性）**

设每子网聚合者集合 $\mathcal{AGG}_i$，$|\mathcal{AGG}_i|=A$，其中拜占庭聚合者数 $< A/3$。若某聚合者丢弃聚合包，则剩余诚实聚合者中至少一个可在超时 $\tau_{\mathrm{agg}}=3\Delta$ 内完成聚合并提交。全网 AttestMsg 丢失率满足：

$$\Pr[\text{子网 }i\text{ 聚合失败}] \leq \exp\!\left(-A \cdot D\!\left(\frac{1}{3}\,\Big\|\,\beta\right)\right)$$

设 $A=9$，$\beta=0.2$：$\Pr[\text{单子网失败}] \leq e^{-9 \times 0.0488} \approx 0.644$。  
但 $S=64$ 子网独立，失败子网比例期望值 $\leq 64 \times 0.644 \approx 41$，需将 $A$ 提升至 $A \geq 20$，使单子网失败率 $\leq e^{-20 \times 0.0488} \approx 0.375$，平均失败子网 $\leq 24$，系统仍可从 $\geq 40$ 个子网获取足够聚合证明。

**推论 15.4**：$A=20$，$S=64$，$\beta \leq 0.2$ 时，至少 $S/2 = 32$ 个子网成功提交聚合证明的概率 $\geq 1 - 2^{-32}$，满足安全门限。

#### 15.1.3 定理 9.4 重写（基于子网聚合）

**定理 9.4'（AttestMsg 聚合后可行性）**

在子网聚合架构下（$S=64$，$A=20$，$n_s=15{,}625$），$N=10^6$ 节点的 AttestMsg 系统满足：

| 指标 | 原方案（裸广播） | 子网聚合方案 | 比值 |
|------|----------------|------------|------|
| 全网消息总量/s | $10^5$ msg/s | $1{,}563$ msg/s | 64× 压缩 |
| 全网带宽需求 | 7.6 MB/s（含 Gossip 放大 50×：380 MB/s） | **6 MB/s**（子网内局部 Gossip） | 63× |
| 单节点处理消息 | 3{,}125 msg/s | **49 msg/s**（子网内） | 64× |
| 聚合者负载 | — | $n_s/A = 781$ msg/s | 新增但可控 |
| 活性保障 | 无结构保障 | 定理 15.3 保证 $\geq 32$ 子网成功 | 有形式保障 |

**证明**：消息量由定理 15.2 推出；带宽由消息量×消息大小（约 120 字节）计算；活性由定理 15.3 给出。$\square$
...
#### 15.3.2 AttestMsg 吞吐基准（验证定理 9.4'）

**实验设置**：
- 32 个 Pool，每 Pool 处理约 $3{,}125$ msg/s（原方案）或 $49$ msg/s（子网聚合方案）
- 单节点硬件：4 核 CPU，16 GB RAM，100 Mbps 上行

**测量指标**：

| 指标 | 原方案目标 | 聚合方案目标 | 测量方法 |
|------|-----------|------------|---------|
| ECDSA 验证吞吐 | 3,125 sig/s | 49 sig/s | `openssl speed ecdsa` |
| CPU 核心占用 | $\leq 2$ 核 | $< 0.1$ 核 | `perf stat` |
| 内存占用（消息缓存） | $\leq 1$ GB | $\leq 32$ MB | `/proc/meminfo` |
| 端到端延迟（从广播到确认） | $\leq 10$s | $\leq 10$s + 聚合延迟 | 日志时间戳 |

**期望结论**：原方案 3,125 sig/s × 32 核 ≈ 100K sig/s，需满配 CPU；子网聚合方案降至 49 sig/s，普通节点单核即可处理，直接论证了百万节点参与的**硬件可达性**。
...
|------|------|---------|---------|-----------|
| **理论形式化** | 子网聚合定理（15.1） | 必须 | 可选 | 论文初稿前完成 |
