...
>
> **密码学底层**：DKG 采用发表于 TNSE（IEEE Transactions on Network Science and Engineering）的动态多项式密钥复用协议。每个委员会成员将 1023 份 ECDH 加密密文**一次性打包为 1 条广播消息**，全委员会共 $k=1024$ 条广播；TNSE 密钥复用使每条消息的多项式承诺载荷降低 **90%**（65 KB→6.5 KB），单轮 DKG 总数据量从约 133 MB 降至约 73 MB，带宽需求从约 700 KB/s 降至约 385 KB/s，彻底消除广域网并发 DKG 通信打穿风险；候选节点注册时提交的验证向量承诺 $c_i=H(V_i)$ 作为离线公钥基底，链上承诺使在线阶段省去完整多项式承诺重传，90% 承诺体积降低正是这一设计的直接结果。
>
> **核心结论**：在 A1–A10 条件下，方案同时达到 BFT 安全性理论极限（$2^{-144}$）、有效 Nakamoto 系数理论上界的 $95.4\%$（31,790）、线性通信复杂度最优（$O(k)$），TPS 与候选池规模完全解耦，且网络规模通过 Root 共识准入限流实现李雅普诺夫渐近稳定收敛（定理 13.16）。
>
...
### C4：TNSE 动态密钥复用协议——DKG 通信量削减 90%

本方案的 DKG 层建立在发表于 IEEE TNSE（Transactions on Network Science and Engineering）的动态多项式密钥复用协议之上，而非裸 Pedersen DKG。这一底层密码学选择对全体理论体系带来结构性影响：

**通信物理可行性的逆转**：本方案 DKG 实现中，每个委员会成员将全部 $k-1=1023$ 份 ECDH 加密密文**一次性打包成 1 条广播消息**，全委员会共产生 $k=1024$ 条广播消息（每条含 1023 份密文 + 多项式承诺）。标准 Pedersen DKG 下，每条消息需携带完整多项式承诺（$t=\lceil 2k/3\rceil = 683$ 个 G2 点，约 65 KB）+ 1023 份 ECDH 密文（约 65 KB），单条消息约 130 KB，1024 条广播总数据量约 133 MB，$T_{\mathrm{bls}}=190$s 窗口内带宽需求约 700 KB/s，在跨洲广域网（$\Delta=1$–$3$s、存在网络抖动）环境下，Complaint 处理引发的重传与附加轮次极易打穿带宽预算。TNSE 密钥复用协议通过**差异化更新**消除对完整多项式承诺的重复广播——每条消息仅携带差异多项式承诺（约 6.5 KB）+ 1023 份 ECDH 密文（约 65 KB），单条消息约 71.5 KB，**每条消息报文体积减少约 45%（多项式承诺部分降低 90%）**，1024 条广播总数据量降至约 73 MB，带宽需求降至约 385 KB/s，万兆/千兆公网轻松承载，$T_{\varphi}=19$s 单阶段窗口对网络毛刺的容忍冗余从临界提升至 **5–8 倍**。

**密钥复用的数学机制**：在候选池 $N=10^5$、每轮随机抽取 $k=1024$ 委员会的架构下，连续两届委员会的期望重合节点数仅为

$$\mathbb{E}[|K_t \cap K_{t+1}|] = k \cdot \frac{k}{N} = 1024 \times \frac{1024}{10^5} \approx 10.5 \text{ 个节点}$$

约 99% 的成员为全新面孔。TNSE 协议实现"99% 成员更替下 90% 通信削减"的关键机制在于：注册阶段每个候选节点 $i$ 向链上提交的验证向量承诺 $c_i = H(V_i)$（第 13.2 节）作为**离线公钥基底**。在线 DKG 阶段，新委员会成员无需重新广播完整多项式承诺，仅需基于已上链的 $c_i$ 执行非交互式份额推导与差异化更新，消除了传统 DKG 最昂贵的"冗余多项式承诺广播交互"环节。

**前向安全性（Proactive Secret Refreshing）**：为防止移动拜占庭对手（Mobile Adversary）跨轮次累积历史份额重构当前主私钥，TNSE 协议每 Epoch 引入零常数项随机多项式扰动：

$$f^{(t+1)}(x) = f^{(t)}(x) + h(x), \quad h(0) = 0$$

在保持主公钥 $\mathrm{PK}_{\mathrm{common}}$ 不变的同时，不可逆刷新全体子份额，对手跨轮次积累的历史份额在当前轮次完全失效。形式化证明见定理 13.15（前向安全性定理）。

**与整体架构的闭环**：TNSE 协议是整条安全链的密码学基础——"链上注册承诺（C4）→ 动态抽样委员会（C1）→ 低通信量 DKG 重构 → BFT 出块（C3）"实现了从注册存储到共识出块的完整贯通，使本方案并非纸上谈兵的同步模型玩具，而是建立在已被验证的低开销动态秘密共享协议之上的工业级共识架构。
...
**定理 9.5（TNSE 密钥复用协议下 DKG 不随 N 扩展）**

采用 TNSE 动态多项式密钥复用协议后，BLS DKG 的通信复杂度从标准 Pedersen DKG 的 $O(k^2)$ 等效降低为 $O(k^2/10)$（通信量削减 90%）；计算复杂度维持 $O(k^2)$，与候选池规模 $N$ 无关。

**预算分配对比**（$k=1024$，$t=\lceil 2k/3\rceil=683$，$T_{\mathrm{bls}}=190$s）：

| 操作 | 标准 Pedersen DKG | TNSE 密钥复用 DKG | 改善 |
|------|-----------------|-----------------|------|
| DKG 广播消息条数 | $k=1{,}024$ 条（每节点 1 条广播） | $k=1{,}024$ 条（不变） | 条数不变 |
| 每条消息有效载荷 | $\approx130$ KB（多项式承诺 65KB + 1023 ECDH 密文 65KB） | $\approx71.5$ KB（差异承诺 6.5KB + 1023 ECDH 密文 65KB）（**-90%**） | 体积降 45%（主要差异在承诺部分 -90%） |
| 单轮 DKG 总数据量 | $\approx133$ MB | $\approx73$ MB（**承诺部分降低 90%**） | $\approx45\%$ 总量下降 |
| 带宽需求（$T_{\mathrm{bls}}=190$s） | $\approx 700$ KB/s | $\approx 385$ KB/s | 带宽需求减半 |
| $T_{\varphi}=19$s 对网络毛刺冗余 | 临界（毛刺即超时） | **5–8 倍冗余余量** | 定理 13.12 获物理支撑 |
| Complaint 风暴带宽压力 | 高（Complaint 轮次叠加原本高负载） | 低（基础带宽压力大幅降低，冗余余量充足） | 活性保障质量提升 |
| Fisher-Yates 抽签（候选池） | $O(N\log N)$ | $O(N\log N)$ | 不变 |
| 委员会固化（M2 的 SampleCommittee） | $O(k)$ | $O(k)$ | 不变 |

**TNSE 协议的通信削减机理**：每个委员会成员将全部 1023 份 ECDH 加密密文**一次性打包为 1 条广播消息**，全委员会共 $k=1024$ 条广播。标准 DKG 要求每条消息携带完整多项式承诺（683 个 G2 点，约 65 KB），因每轮委员会均为新组合；TNSE 协议利用候选节点注册时已上链的验证向量承诺 $c_i=H(V_i)$ 作为**离线公钥基底**，在线 DKG 阶段仅广播差异多项式承诺（约 6.5 KB），多项式承诺部分降低 90%，每条消息总报文体积减少约 45%，总带宽需求减半，从而彻底消除 Complaint 风暴打穿 $T_{\mathrm{bls}}$ 窗口的风险。

**EC1 约束重设**：引入 TNSE 协议后，DKG 带宽压力从约 700 KB/s 降至约 385 KB/s，$T_{\varphi}=19$s 的单阶段窗口对公网偶发网络毛刺（Jitter）具备 5–8 倍冗余容忍度，定理 13.12 的 $P_{\mathrm{DKG}} \geq 99\%$ 在真实广域网环境中真正获得物理可行性支撑。

**结论**：将候选池从 1024 扩展到 $10^5$ 仅在选举结算时增加 $O(N)$ 一次性统计工作（FTS 分值计算），**DKG 广播消息条数恒为 $k=1024$，不随 $N$ 增加**；TNSE 密钥复用协议将每条消息的承诺载荷压缩 90%，进一步将总带宽需求降低约 45%，使在线 DKG 通信压力降至公网可轻松承载的水平。
...
#### 15.3.1 TNSE-DKG 压力测试（验证定理 13.12-TNSE）
