...
> 子分片结构：每节点确定性加入 $J=10$ 个子分片，每子分片容量 $k_{\mathrm{pool}}=1024$（根分片共识强制），形成 $P=\lceil N\cdot J/k_{\mathrm{pool}}\rceil=977$ 个子分片
>
...
| **块广播效率**：委员会（$k=1024$）需将 QC 块高效广播到 $N=10^5$ 候选节点 | 直接 Gossip 需 $O(N)$ 条消息，委员会出口带宽过高 | M6 随机中继广播：随机选 $R$ 个节点，各向其 $J=10$ 个子分片注入块，$R \times J$ 次定向发送覆盖 $P=977$ 子分片，零新增基础设施 | 定理 9.4 |
...
| 通信架构 | 全员广播，$O(N)$ | 子网聚合（ETH2 设计基础） | **M6：随机中继广播，$R$ 节点 × $J=10$ 子分片 = $O(R)$ 消息覆盖全网，复用现有子分片拓扑** |
...
**背景**：委员会（$k=1024$）生成 QC 块后，需高效将块广播到全部 $N=10^5$ 候选节点。直接从委员会向全网 Gossip 需 $O(\log N)$ 跳、$O(N)$ 条消息。M6 利用**现有分片池 P2P 拓扑**，通过随机中继节点将块定向注入子分片网络，实现 $O(R \cdot J)$ 条消息覆盖全网，**零新增基础设施**。

**构造定义（基于现有子分片实现）**：

**（1）确定性池分配**：候选节点 $i$ 的 $J=10$ 个子分片编号由节点 ID 确定性计算：

$$\mathtt{pool\_id}_{i,j} = H(\mathtt{node\_id}_i \,\|\, j) \bmod P, \quad j = 0,\ldots,J-1$$

其中 $P = \lceil N \cdot J / k_{\mathrm{pool}} \rceil = 977$ 为子分片总数，由根分片动态管理。节点无法自选子分片（防止 Sybil 集中）。

**（2）根分片容量控制（均衡保障）**：每个子分片节点数上限 $k_{\mathrm{pool}}=1024$，由根分片共识在 JoinElectTx 中强制执行：

$$\mathrm{JoinElectTx}_i \text{ 共识通过} \iff |\mathcal{M}_p| < k_{\mathrm{pool}},\quad \forall p \in \mathtt{pools}(i)$$

容量上限超限时交易共识失败，不进入候选池。此机制保证所有子分片节点数均衡（每子分片 $\leq 1024$ 节点）且无需中心化协调。

**（3）随机中继选举**：每块由委员会从候选池随机选取 $R$ 个中继节点（复用 M2 同一信标）：

$$\mathcal{R} = \mathrm{FisherYates}(\mathcal{C}_T,\,R,\,\mathtt{epoch\_random\_} \oplus \mathtt{block\_height})$$

**（4）子分片定向广播**：委员会向 $\mathcal{R}$ 中的 $R$ 个节点单播块，每个中继节点将块广播到其所属的 $J=10$ 个子分片，复用现有 P2P 子分片连接，**零新增基础设施**。

$$\text{覆盖广播数} = R \times J = 100 \times 10 = 1000 \text{ 次},\quad P = 977 \text{ 个子分片几乎完全覆盖}$$

**参数**：$J=10$，$k_{\mathrm{pool}}=1024$，$R=100$（生产建议 $R=200$ 覆盖率 89%）。

**关键优势**：无 BLS 聚合逻辑，无 AggregatedAttest，无 bitmask。中继节点收到块后走现有块同步路径（`key_value_sync.cc`），对拜占庭中继容忍依赖未覆盖子分片的 pull 同步兜底。
...
| **M6 子分片广播** | **块广播效率** | **随机中继 $R$ 个节点 + 各节点向 $J=10$ 子分片定向注入** | **$R=100$，$J=10$，$k_{\mathrm{pool}}=1024$** |
...
本节基于**机制 M6（随机中继广播）**分析块从委员会向全体 $N=10^5$ 候选节点传播的通信复杂度。M6 复用现有分片 P2P 拓扑（每节点已参与 $J=10$ 个子分片网络），**零新增连接开销**，以 $O(R \cdot J)$ 条定向消息覆盖 $P=977$ 个子分片。

**定理 9.4（随机中继广播通信复杂度，M6 架构）**

设候选池 $N=10^5$，子分片数 $P=977$，每节点参与子分片数 $J=10$，中继节点数 $R$，拜占庭比例 $\beta < 1/3$。

**（a）委员会发送负载**：

委员会向 $R$ 个中继节点单播块，发送量与 $N$ 无关：

$$V_{\mathrm{committee}} = R \text{ 条单播（固定常数，}R=100\text{ 至 }500\text{）}$$

**（b）子分片覆盖率**：

$R$ 个中继节点各覆盖 $J=10$ 个子分片，共 $R \cdot J$ 次子分片注入。未被直接覆盖的子分片期望数：

$$\mathbb{E}[\text{未覆盖}] = P \cdot \left(1 - \frac{1}{P}\right)^{R \cdot J} \approx P \cdot e^{-R \cdot J / P}$$

对 $R=200$，$J=10$，$P=977$：期望未覆盖 $\approx 977 \cdot e^{-2000/977} \approx 977 \cdot e^{-2.05} \approx 110$ 个子分片（覆盖率约 89%）。

**（c）容错性**：

$R$ 个中继节点中含 $\leq \beta R$ 个拜占庭节点（$\beta < 1/3$），诚实中继覆盖 $\geq (1-\beta) R \cdot J = 0.8 \times 200 \times 10 = 1600$ 次子分片注入；未覆盖子分片通过现有 pull 同步（`key_value_sync.cc`）在下一出块间隔内追上。$\square$

**证明**：(a) 直接由 $R$ 的定义。(b) 每次注入独立选择子分片（哈希均匀），$R \cdot J$ 次覆球问题（coupon collector 变体）。(c) 诚实中继数 $\geq (1-\beta)R$；pull 同步在 $t_b=10$s 内完成（已有实现）。$\square$

**对比（直接 Gossip vs M6 随机中继，$N=10^5$）**：

| 指标 | 直接 Gossip | M6 随机中继 | 改善 |
|------|------------|-----------|------|
| 委员会发送消息数/块 | $O(N)=10^5$ 条 | $R=200$ 条 | $500\times$ |
| 块传播跳数 | $O(\log N)\approx 17$ 跳 | 2 跳（委员会→中继→子分片） | 显著降低 |
| 新增 P2P 连接 | — | 零（复用现有子分片拓扑） | 零新增 |
| 拜占庭容忍 | 网络层 Gossip 天然容错 | 诚实中继 + pull 同步兜底 | 等效 |
...
设候选池 $N=10^5$，每节点参与子分片数 $J=10$，子分片容量 $k_{\mathrm{pool}}=1024$，则由根分片 JoinElectTx 共识动态维护的子分片数满足：

$$P = \left\lceil \frac{N \cdot J}{k_{\mathrm{pool}}} \right\rceil = \left\lceil \frac{10^5 \times 10}{1024} \right\rceil = 977$$

每子分片平均成员数为 $N \cdot J / P = 10^5 \times 10 / 977 \approx 1023 \leq k_{\mathrm{pool}}$，满足容量约束。

**委员会 BLS 批量验证代价**：每块验证 $P=977$ 条聚合签名，采用随机线性组合批量验证（BLS batch）：
...
| 子分片容量（定理 9.7） | $P \cdot k_{\mathrm{pool}} \geq N \cdot J$ | $977 \times 1024 = 1.0\times10^6 = N\cdot J$ ✓ |
...
| 委员会聚合验证 | $P=977$ 次 BLS 批量 | $O(1)$ 配对，$\ll t_b$（定理 9.7）|
...
| 随机委员会 $k=1024,N=10^6$ | 342 | **317,901** | **930×**（参考） |
...
节点 $i$ 提交 `JoinElectTx`，其中包含目标子分片集合 $\mathtt{pools}(i) = \{p_0,\ldots,p_{J-1}\}$（由 $\mathtt{node\_id}_i$ 确定性计算）。根分片委员会在共识时验证：

$$\forall p \in \mathtt{pools}(i): |\mathcal{M}_p^{\mathrm{current}}| < k_{\mathrm{pool}}$$

若任意目标子分片已满（$|\mathcal{M}_p| \geq k_{\mathrm{pool}}$），则交易共识**失败**，节点需等待下一 Epoch 重试（届时可能有节点退出，释放容量）。
...
在均衡状态下（$N$ 个节点全部成功注册），子分片总数满足：

$$P \geq \left\lceil \frac{N \cdot J}{k_{\mathrm{pool}}} \right\rceil$$

**数值**（$N=10^5$，$J=10$，$k_{\mathrm{pool}}=1024$）：$P \geq 977$，每子分片约 1023 个节点，各子分片节点数在 $[N\cdot J/P - \delta, k_{\mathrm{pool}}]$ 之间（$\delta$ 为入退差异引起的小波动）。这保证了 M6 广播分析中"各子分片成员数均衡"的基本假设成立。$\square$

**定理 13.0''（确定性分配下的拜占庭比例上界）**

设拜占庭节点总数 $B \leq \beta N$（$\beta < 1/3$），节点分配由 $\mathtt{pool\_id}_{i,j} = H(\mathtt{node\_id}_i \| j) \bmod P$ 确定性给出。攻击者可通过枚举 $\mathtt{node\_id}$ 将拜占庭节点集中于少数子分片，但 JoinElectTx 容量控制保证：

$$\forall p: |\mathcal{B} \cap \mathcal{M}_p| \leq k_{\mathrm{pool}} = 1024$$

在候选池压力充足（$N \cdot J \gg P \cdot k_{\mathrm{pool}}$，即加入竞争激烈）的条件下，诚实节点与拜占庭节点按比例竞争子分片容量，任意子分片内的拜占庭比例期望值为 $\beta$：

$$\mathbb{E}\!\left[\frac{|\mathcal{B} \cap \mathcal{M}_p|}{|\mathcal{M}_p|}\right] = \beta, \quad \forall p$$

**证明**：总加入竞争数 $N \cdot J = 10^6$，总容量 $P \cdot k_{\mathrm{pool}} \approx 10^6$，竞争比约 1:1。在诚实节点先到先得（非恶意）的假设下，子分片填满过程中诚实/拜占庭节点的到达率比例为 $(1-\beta):\beta$，期望比例为 $\beta$。若攻击者集中投入 $B$ 个节点至 $q$ 个目标子分片，每子分片平均拜占庭数为 $B/q$；但每子分片最多容纳 $k_{\mathrm{pool}}$ 节点，诚实节点也会填满这 $q$ 个子分片，使拜占庭比例收敛至 $B/(q \cdot k_{\mathrm{pool}})$。当 $q = B/k_{\mathrm{pool}} = \beta N/k_{\mathrm{pool}} = 97.7$ 时，攻击者可使这 97.7 个子分片的拜占庭比例达到 100%，但剩余 $P - 97.7 = 879$ 个子分片拜占庭比例接近 0。系统聚合时使用的是所有 $P$ 个子分片，因此整体平均拜占庭比例仍为 $\beta$，不影响全局安全性。$\square$
...
**已解决**：M6 采用随机中继广播方案（$N=10^5$，$J=10$，$k_{\mathrm{pool}}=1024$，$R=100$~$500$），委员会仅向 $R$ 个中继节点单播，中继节点通过现有子分片拓扑将块注入 $P=977$ 个子分片。委员会发送量从 $O(N)$ 降至 $O(R)$，**无 BLS 聚合逻辑**，无大规模 Gossip 扩散。定理 9.4 已更新为基于随机中继方案的精确分析。
...
设候选池 $N=10^5$，$P=977$ 个子分片，每节点参与 $J=10$ 个子分片（确定性分配），每子分片容量 $k_{\mathrm{pool}}=1024$（根分片共识强制）。每块通过以下机制传播：

1. **中继选举**：$\mathcal{R} = \mathrm{FisherYates}(\mathcal{C}_T, R, \mathtt{epoch\_random\_} \oplus \mathtt{block\_height})$，复用 M2 信标随机性
2. **定向注入**：委员会向 $|\mathcal{R}|=R$ 个节点单播块，每个中继节点将块广播到其所属 $J=10$ 个子分片
3. **子分片传播**：各子分片 $\leq 1024$ 个成员通过现有 P2P Gossip 接收块
4. **兜底同步**：未被直接覆盖的子分片成员通过 pull 同步（`key_value_sync.cc`）追块

**定理 15.2（块广播覆盖率）**

在定义 15.1 的协议下，$R$ 个中继节点覆盖的子分片期望数满足：

$$\mathbb{E}[\text{覆盖子分片数}] = P \cdot \left(1 - \left(1 - \frac{1}{P}\right)^{R \cdot J}\right) \geq P \cdot \left(1 - e^{-R \cdot J / P}\right)$$

对 $R=200$，$J=10$，$P=977$：覆盖率 $\geq 89\%$；对 $R=500$：覆盖率 $\geq 99.8\%$。$\square$

**证明**：每次子分片注入独立均匀选择一个子分片（哈希均匀性），$R \cdot J$ 次注入后某子分片未被覆盖概率 $= (1-1/P)^{R \cdot J} \leq e^{-R \cdot J/P}$；对 $P$ 个子分片求期望。$\square$

#### 15.1.2 容错性分析

**定理 15.3（拜占庭中继容忍）**

设 $R$ 个中继节点中拜占庭比例 $\leq \beta < 1/3$，则诚实中继数 $\geq (1-\beta) R$，诚实中继覆盖子分片期望数：

$$\mathbb{E}[\text{诚实覆盖}] \geq P \cdot \left(1 - e^{-(1-\beta) R \cdot J / P}\right)$$

未被直接覆盖的子分片通过 pull 同步在 $t_b=10$s 内完成追块（已有实现保证）。$\square$
...
| 容错兜底 | Gossip 天然 | pull 同步（已有实现） | 等效 |
...
**实验设置**：
- $N=10^5$ 候选节点，$J=10$，$k_{\mathrm{pool}}=1024$，$R=100$~$500$
- 对比：直接 Gossip（委员会向全网广播）vs. M6 随机中继广播（$R$ 个中继节点）
- 单节点硬件：4 核 CPU，16 GB RAM，100 Mbps 上行

**测量指标**：

| 指标 | 直接 Gossip | M6 随机中继（$R=200$） | 测量方法 |
|------|-----------|----------------------|---------|
| 委员会出向消息数/块 | $O(N)=10^5$ 条 | $R=200$ 条 | 日志计数 |
| 块平均传播时间（全网 95%） | 基线 | 目标 $\leq t_b/2=5$s | 时间戳 |
| 未覆盖子分片比例 | 0%（全量） | $\leq 11\%$（$R=200$）→ pull 兜底 | 覆盖率统计 |
| pull 同步触发频率 | 无需 | $\leq 11\% \times P=107$ 次/块 | 日志 |

**期望结论**：M6 随机中继广播使委员会发送量从 $O(N)$ 降至 $O(R)$，块传播在 2 跳内完成（委员会→中继→子分片），未覆盖子分片通过 pull 同步兜底，整体延迟在 $t_b=10$s 内可控，且无 BLS 聚合逻辑，实现最简化。
