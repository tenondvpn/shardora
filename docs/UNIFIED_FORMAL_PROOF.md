...
| **块广播效率**：委员会（$k=1024$）需将 QC 块高效广播到 $N=10^5$ 候选节点 | 直接 Gossip 需 $O(N)$ 条消息，委员会出口带宽过高 | M6 随机中继广播：委员会每子分片选 1 个中继，各向其所属子分片注入块，$R=P=100$ 次定向发送覆盖全部 $P=100$ 子分片，零新增基础设施 | 定理 9.4 |
...
| 通信架构 | 全员广播，$O(N)$ | 子网聚合（ETH2 设计基础） | **M6：随机中继广播，委员会每子分片选 1 个中继，$R=P=100$ 条消息覆盖全网，复用现有子分片拓扑** |
...
**背景**：委员会（$k=1024$）生成 QC 块后，需高效将块广播到全部 $N=10^5$ 候选节点。直接从委员会向全网 Gossip 需 $O(\log N)$ 跳、$O(N)$ 条消息。M6 利用**现有分片池 P2P 拓扑**，通过随机中继节点将块定向注入子分片网络，实现 $R=P=100$ 条消息覆盖全网，**零新增基础设施**。
...
其中 $P = \lceil N / k_{\mathrm{pool}} \rceil = 100$ 为子分片总数，由根分片动态管理。节点无法自选子分片（防止 Sybil 集中）。
...
**（3）随机中继选举**：委员会从每个子分片各选取 1 个中继节点（复用 M2 同一信标）：

$$\mathcal{R} = \{\mathrm{FisherYates}(\mathcal{M}_p, 1, \mathtt{epoch\_random\_} \oplus p) : p=0,\ldots,P-1\}$$

共 $R=P=100$ 个中继节点（每子分片恰好 1 个）。

**（4）子分片定向广播**：委员会向 $\mathcal{R}$ 中的 $R=P=100$ 个节点单播块，每个中继节点将块广播到其所属的子分片，复用现有 P2P 子分片连接，**零新增基础设施**。

$$R = P = 100,\quad \text{每子分片恰好 1 个中继，覆盖率 100\%}$$

**参数**：$k_{\mathrm{pool}}=1024$，$P=100$，$R=P=100$（每子分片 1 个中继）。
...
| **M6 子分片广播** | **块广播效率** | **每子分片选 1 个中继节点，委员会发 $R=P=100$ 条消息，各中继广播到其子分片** | **$P=100$，$R=100$，$k_{\mathrm{pool}}=1024$** |
...
本节基于**机制 M6（随机中继广播）**分析块从委员会向全体 $N=10^5$ 候选节点传播的通信复杂度。M6 复用现有分片 P2P 拓扑（每节点属于 1 个子分片），**零新增连接开销**，以 $R=P=100$ 条定向消息覆盖全部 $P=100$ 个子分片。

**定理 9.4（随机中继广播通信复杂度，M6 架构）**

设候选池 $N=10^5$，子分片数 $P=\lceil N/k_{\mathrm{pool}}\rceil=100$，每节点属于 1 个子分片，$R=P=100$ 个中继节点（每子分片 1 个），拜占庭比例 $\beta < 1/3$。

**（a）委员会发送负载**：

委员会向 $R=P=100$ 个中继节点单播块，发送量与 $N$ 无关：

$$V_{\mathrm{committee}} = R = P = 100 \text{ 条单播（固定常数）}$$

**（b）子分片覆盖率**：

委员会每子分片选 1 个中继节点，共 $R=P=100$ 次子分片注入，每子分片恰好被覆盖 1 次：

$$\text{覆盖率} = 100\% \text{（确定性，每子分片恰好 1 个中继）}$$

**（c）容错性**：

若某子分片的中继节点为拜占庭节点，该子分片成员通过现有 pull 同步（`key_value_sync.cc`）在下一出块间隔内追上。$\square$

**证明**：(a) 直接由 $R=P$ 的定义。(b) 每子分片恰好分配 1 个中继，覆盖率确定性为 100%。(c) 拜占庭中继节点导致对应子分片未收到块，pull 同步在 $t_b=10$s 内完成（已有实现）。$\square$

**对比（直接 Gossip vs M6 随机中继，$N=10^5$）**：

| 指标 | 直接 Gossip | M6 随机中继 | 改善 |
|------|------------|-----------|------|
| 委员会发送消息数/块 | $O(N)=10^5$ 条 | $R=P=100$ 条 | $1000\times$ |
| 块传播跳数 | $O(\log N)\approx 17$ 跳 | 2 跳（委员会→中继→子分片） | 显著降低 |
| 子分片覆盖率 | — | 100%（确定性） | 确定性保证 |
| 新增 P2P 连接 | — | 零（复用现有子分片拓扑） | 零新增 |
| 拜占庭容忍 | 网络层 Gossip 天然容错 | 拜占庭中继 → pull 同步兜底 | 等效 |
...
$$P = \left\lceil \frac{N}{k_{\mathrm{pool}}} \right\rceil = \left\lceil \frac{10^5}{1024} \right\rceil = 100$$

每子分片平均成员数为 $N / P = 10^5 / 100 = 1000 \leq k_{\mathrm{pool}}$，满足容量约束。
...
$$P \geq \left\lceil \frac{N}{k_{\mathrm{pool}}} \right\rceil$$

**数值**（$N=10^5$，$k_{\mathrm{pool}}=1024$）：$P \geq 100$，每子分片约 1000 个节点，各子分片节点数在 $[N/P - \delta, k_{\mathrm{pool}}]$ 之间（$\delta$ 为入退差异引起的小波动）。这保证了 M6 广播分析中"各子分片成员数均衡"的基本假设成立。$\square$
...
**注**：当前代码（`JoinElectInfo.g2_req`）将完整验证向量 $V_i$ 放入交易体。本协议改为仅上链 32 字节承诺 $c_i$，节省 $43{,}712 - 32 = 43{,}680$ 字节/节点的链上存储；对 $N=10^5$ 候选节点节省 $\approx 4.2$ TB 链上负担。
...
**已解决**：M6 采用随机中继广播方案（$P=100$，$k_{\mathrm{pool}}=1024$，$R=P=100$），委员会每子分片选 1 个中继节点共 $R=100$ 个，通过现有子分片拓扑将块注入 $P=100$ 个子分片，覆盖率确定性为 100%。委员会发送量从 $O(N)$ 降至 $O(R)$，**无 BLS 聚合逻辑**，无大规模 Gossip 扩散。定理 9.4 已更新为基于随机中继方案的精确分析。
...
1. **中继选举**：$\mathcal{R} = \{\mathrm{FisherYates}(\mathcal{M}_p, 1, \mathtt{epoch\_random\_} \oplus p) : p=0,\ldots,P-1\}$，复用 M2 信标随机性，每子分片各选 1 个中继，共 $R=P=100$ 个
2. **定向注入**：委员会向 $|\mathcal{R}|=R=P=100$ 个节点单播块，每个中继节点将块广播到其所属子分片
3. **子分片传播**：各子分片 $\leq 1024$ 个成员通过现有 P2P Gossip 接收块
4. **兜底同步**：拜占庭中继所在子分片成员通过 pull 同步（`key_value_sync.cc`）追块

**定理 15.2（块广播覆盖率）**

在定义 15.1 的协议下，$R=P=100$ 个中继节点（每子分片 1 个）覆盖的子分片数满足：

$$\text{覆盖子分片数} = P = 100 \quad (\text{确定性，覆盖率 } 100\%)$$

每子分片恰好被分配 1 个中继节点，覆盖率确定性为 100%，无需概率分析。$\square$

**证明**：由定义 15.1 第 1 步，对每个 $p \in \{0,\ldots,P-1\}$ 均从 $\mathcal{M}_p$ 中选出 1 个中继节点，共 $P$ 个子分片每个恰好被覆盖 1 次，覆盖率确定性为 100%。$\square$

#### 15.1.2 容错性分析

**定理 15.3（拜占庭中继容忍）**

设 $R=P=100$ 个中继节点（每子分片 1 个）中拜占庭比例 $\leq \beta < 1/3$，则诚实中继数 $\geq (1-\beta) R$，对应子分片直接覆盖；拜占庭中继所在的至多 $\beta R$ 个子分片通过 pull 同步在 $t_b=10$s 内完成追块（已有实现保证）。$\square$
...
| 容错兜底 | Gossip 天然 | 拜占庭中继 → pull 同步（已有实现） | 等效 |
...
**实验设置**：
- $N=10^5$ 候选节点，$P=100$，$k_{\mathrm{pool}}=1024$，$R=P=100$
- 对比：直接 Gossip（委员会向全网广播）vs. M6 随机中继广播（$R=P=100$ 个中继节点）
- 单节点硬件：4 核 CPU，16 GB RAM，100 Mbps 上行

**测量指标**：

| 指标 | 直接 Gossip | M6 随机中继（$R=P=100$） | 测量方法 |
|------|-----------|----------------------|---------|
| 委员会出向消息数/块 | $O(N)=10^5$ 条 | $R=P=100$ 条 | 日志计数 |
| 块平均传播时间（全网 95%） | 基线 | 目标 $\leq t_b/2=5$s | 时间戳 |
| 未覆盖子分片比例 | 0%（全量） | 0%（确定性）→ 拜占庭中继时 pull 兜底 | 覆盖率统计 |
| pull 同步触发频率 | 无需 | 仅拜占庭中继时触发（$\leq \beta P$ 次/块） | 日志 |

**期望结论**：M6 随机中继广播使委员会发送量从 $O(N)$ 降至 $O(R)=O(P)=O(100)$，块传播在 2 跳内完成（委员会→中继→子分片），覆盖率确定性为 100%，拜占庭中继通过 pull 同步兜底，整体延迟在 $t_b=10$s 内可控，且无 BLS 聚合逻辑，实现最简化。
