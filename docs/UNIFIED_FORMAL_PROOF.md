...
**定理 13.15-OPT（Pareto 最优更替率定理）**

> **背景**：定理 13.15' 对任意 $\rho_{\rm churn}$ 给出份额暴露停时上界，但并未揭示 $\rho_{\rm churn}$ 的最优取值。本定理从密码学安全、自适应对手抑制、DKG 带宽三个维度构造 Pareto 优化问题，精确导出理论最优 $\rho^*=25\%$。

**优化问题形式化**：设 $k=1024$，$t=\lceil k/3\rceil=342$，$T_{\rm bls}=190$s，$B_{\rm WAN}=100$Mbps，寻找：

$$\rho^* = \arg\min_{\rho\in(0,1]}\; C_{\rm comm}(\rho) \quad \text{s.t.} \begin{cases} \mathbf{C_1:} & (1-\rho)^{342} \leq 2^{-128} \\ \mathbf{C_2:} & 1/\rho \leq \tau_{\max} \\ \mathbf{C_3:} & B_{\rm peak}(\rho) \leq B_{\rm WAN} \end{cases}$$

**约束 C1 的精确解（密码学 128-bit 安全红线）**：

$$(1-\rho)^{342} \leq 2^{-128} \;\Longleftrightarrow\; \rho \geq 1 - e^{-128\ln 2/342} = 1 - e^{-0.25942} \approx 1 - 0.77149 = 22.85\%$$

即 $\rho < 22.85\%$ 的任何方案均无法在无外部退火假设的情况下原生满足 128-bit 移动对手安全性。取 $\rho=10\%$ 时：$(0.9)^{342}\approx2^{-52}$，仅有约 52-bit 安全，显著低于密码学顶会标准红线。

**约束 C2 的意义（自适应物理渗透窗口）**：在任期 $\mathbb{E}[\tau]=1/\rho$ 个 Epoch 内，物理对手完成机器渗透后拥有合法投票权。取 $\tau_{\max}=4$ 个 Epoch（$=2400$s $=40$ 分钟）可将自适应立足点暴露时间压缩至 $\rho=10\%$ 基准的 40%，即 $\rho\geq25\%$。

**约束 C3 的验证（广域网带宽可达）**：TNSE 差量多项式承诺阶数随 $\rho$ 线性增长，单节点广播报文：

$$\text{msg}(\rho) = \underbrace{(k-1)\times64\text{ B}}_{\approx65.5\text{ KB（固定）}} + \underbrace{\rho\cdot t\times96\text{ B}}_{\text{差量承诺}}$$

取 $\rho^*=25\%$：$\text{msg}(0.25) = 65.5 + 0.25\times683\times96/1024\approx80.0\text{ KB}$，全委员会总数据量 $\approx80\text{ MB}$，$T_{\rm bls}=190$s 内平均带宽需求 $\approx3.53$ Mbps，远低于 $B_{\rm WAN}=100$ Mbps，满足 C3。

**定理（结论）**：在 $k=1024$、$t=342$、$N=10^5$ 的 PPKG 协议中，最优更替率为：

$$\boxed{\rho^* = 25\% = \frac{2^8}{2^{10}} \quad \left(k_{\rm new}=256,\; k_{\rm ret}=768\right)}$$

在此参数下系统同时满足：

1. **128-bit 移动抗性**（无退火依赖）：
$$\Pr[\tau_i \geq 342] \leq (0.75)^{342} \approx 2^{-141.9} \ll 2^{-128}$$

2. **稳态全局自洽（精确闭合）**：由于 $k_{\rm new}=256$ 精确整除 $k=1024$，更新方程稳态参与率与全局采样率完全重合：
$$\bar{p}_i = \frac{1/\rho^*}{1/\rho^* + (N-k)/k_{\rm new}} = \frac{4}{4 + 98976/256} = \frac{4}{4 + 386.625} = \frac{k}{N} = 1.024\%$$
（注：$\rho=10\%$ 时 $k_{\rm new}=102\neq 1024\times0.1$，舍入导致微小不自洽；$\rho^*=25\%$ 消除此误差）

3. **差量 DKG 承诺体积降低 75%**（相比全量 Pedersen DKG）：差量承诺阶数 $\lfloor\rho^*\cdot t\rfloor=170$，承诺体积 $170\times96/1024\approx15.9$ KB，全量为 $683\times96/1024\approx63.9$ KB，降低 $1-170/683\approx75\%$。

4. **平均在位时间 40 分钟**：$\mathbb{E}[\tau]=1/0.25=4$ 个 Epoch $=2400$s，自适应对手物理渗透窗口相比 $\rho=10\%$（100 分钟）压缩 60%。

| 指标 | $\rho=10\%$（工程基准） | $\rho^*=25\%$（理论最优） | $\rho=100\%$（传统 DKG） |
|:---|:---:|:---:|:---:|
| $k_{\rm new}$ | 102 | **256** ($=2^8$) | 1024 |
| 停时安全位数 | $\approx52$ bit（弱） | $\approx142$ bit（**超 128-bit**） | $\infty$ |
| 平均在位时间 | 100 min | **40 min** | 10 min |
| 差量承诺体积节省 | 90% | **75%** | 0% |
| 报文数节省（含引导） | 74% | **40%** | 0% |
| 稳态方程精确闭合 | 否（舍入误差） | **是** | — |
| 二进制对齐 | 否（102） | **是**（$2^8$） | 是（$2^{10}$） |

$\square$

> **工程建议**：当前代码参数 `kMaxConsecutiveElections=3`、`k_new≈102` 使用 $\rho=10\%$，在系统上线稳态后应迁移至 $\rho^*=25\%$（$k_{\rm new}=256$），以满足 128-bit 密码学安全标准，同时保留约 75% 的承诺体积节省。参数修改仅涉及 `elect_tx_item.cc` 中的 `kChurnRate` 常量。

---

#### 13.11.4 新委员会成员份额初始化（可验证重分享子协议）

> **设计前提**：系统常规运行模式为每轮淘汰 $k_{\rm new}\approx102$（10%）个委员会成员，从候选池 $N=10^5$ 中补入 102 人，委员会始终维持 $k=1024$ 人。留任的 $k_{\rm ret}\approx922$ 人其份额在上一轮 TNSE 已更新，无需重新引导；本节仅处理 $k_{\rm new}$ 个新成员的份额初始化。

**安全警告（对朴素 Lagrange 方案的勘误）**：若旧成员直接将自身持有的原始份额 $sk_i^{(t)}=F^{(t)}(i)$ 发送给新成员 $j$，则 $j$ 收集 $t$ 个形如 $(i,\,F^{(t)}(i))$ 的点对后，可完整重构多项式 $F^{(t)}(x)$，进而恢复 $F^{(t)}(0)=sk_{\rm common}$。ECDH 信道加密仅保护传输机密性，不改变解密后的信息量，因此无法缓解此安全漏洞。正确协议须使旧成员发送的内容在信息论意义上不足以还原任何原始份额。

---

**引理 13.T4（可验证重分享子协议与份额引导安全性）**

**协议构造**：设 $S\subseteq K_t$，$|S|=t+1$（任意 $t+1$ 个愿意配合的旧委员会成员），新成员 $j\in K_{t+1}\setminus K_t$ 需要获得份额 $sk_j^{(t)}=F^{(t)}(j+1)$。

**步骤 1（旧成员生成贡献多项式）**：对每个 $i\in S$，旧成员 $i$ 均匀随机选取系数 $b_{i,1},\ldots,b_{i,t-1}\in\mathbb{Z}_p$，构造新随机多项式：

$$q_i(x) = sk_i^{(t)} + b_{i,1}x + b_{i,2}x^2 + \cdots + b_{i,t-1}x^{t-1}$$

满足 $q_i(0)=sk_i^{(t)}$，且 $q_i$ 与旧多项式 $F^{(t)}$ 除零次项外无任何结构关联。

**步骤 2（ECDH 加密单播）**：旧成员 $i$ 对每个新成员 $j\in K_{\rm new}$ 计算：
- ECDH 共享密钥 $K_{ij}=\mathrm{ECDH}(sk_i^{\rm EC},\,pk_j^{\rm EC})$
- 发送密文 $\mathrm{Enc}(K_{ij},\;q_i(j+1))$，即多项式 $q_i$ 在新成员 $j$ 位置 $j+1$ 处的求值

**步骤 3（新成员聚合）**：新成员 $j$ 用私钥解密得 $\{q_i(j+1)\}_{i\in S}$，计算 Lagrange 权重合：

$$sk_j^{(t)} = \sum_{i\in S} \lambda_i^{S,0}\cdot q_i(j+1)$$

其中 $\lambda_i^{S,0}=\prod_{l\in S,\,l\neq i}\frac{-l}{i-l}$ 为在点集 $S$ 上对 $x=0$ 插值的 Lagrange 系数。

---

**定理（协议正确性）**：$sk_j^{(t)}$ 等于 $F^{(t)}(j+1)$，即新成员获得了对旧主多项式的合法份额。

**证明**：令 $R(x)=\sum_{i\in S}\lambda_i^{S,0}\cdot q_i(x)$，则 $R$ 是 $\mathbb{Z}_p[x]$ 上的 $t-1$ 次多项式。验证：
...
最后一步利用了 Lagrange 插值恒等式（$|S|=t+1$ 个点唯一确定 $t-1$ 次多项式 $F^{(t)}$）。因此 $R(0)=sk_{\rm common}$，且：

$$sk_j^{(t)}=R(j+1)=\sum_{i\in S}\lambda_i^{S,0}\cdot q_i(j+1)=F^{(t)}(j+1)\qquad\square$$

> **注**：此处 $j+1$ 代入 $R(x)$ 只给出 $F^{(t)}$ 在 $j+1$ 处的一个值，即 $j$ 的合法份额；$j$ 无法通过 $R(j+1)$ 单点值还原 $R(x)$ 全体（需要 $t+1$ 个不同点），故主私钥仍然安全。

---

**定理（协议安全性）**：在步骤 2 中，新成员 $j$ 收到的是每条多项式 $q_i$ 在**自身位置** $j+1$ 处的单点求值，无法从中恢复任何 $q_i(0)=sk_i^{(t)}$，也无法重构 $sk_{\rm common}$。

**证明**：对固定的 $i\in S$，$q_i(x)$ 的 $t-1$ 个系数 $b_{i,1},\ldots,b_{i,t-1}$ 由旧成员 $i$ 均匀随机选取，新成员 $j$ 仅观测到一个方程 $q_i(j+1)=sk_i^{(t)}+b_{i,1}(j+1)+\cdots+b_{i,t-1}(j+1)^{t-1}$，其中 $t-1$ 个自由系数完全未知。由信息论，在不知道 $(b_{i,1},\ldots,b_{i,t-1})$ 的情形下，$q_i(0)=sk_i^{(t)}$ 的条件分布在 $\mathbb{Z}_p$ 上均匀，即 $j$ 对 $sk_i^{(t)}$ 无任何信息增益：

$$H(sk_i^{(t)}\mid q_i(j+1))=H(sk_i^{(t)})=\log_2 p$$

对所有 $i\in S$ 均成立（各 $q_i$ 的随机性独立），故 $j$ 无法恢复 $\{sk_i^{(t)}\}_{i\in S}$，也无法恢复 $sk_{\rm common}=\sum_{i\in S}\lambda_i^{S,0}\cdot sk_i^{(t)}$。$\square$

---

**通信复杂度（正常 10% churn，$k_{\rm new}=102$）**

委员会保持 $k=1024$ 人，每轮正常替换约 $k_{\rm new}=102$ 人，$k_{\rm ret}=922$ 人留任。完整单轮通信由两部分组成：

**（A）份额引导（本引理，仅新成员需要）**：每个旧成员 $i\in S$（取 $|S|=t+1=684$ 人）对所有 $k_{\rm new}=102$ 个新成员各发送一条加密贡献；共计：

$$C_{\rm bootstrap}=|S|\times k_{\rm new}=684\times102=69{,}768\text{ 条}$$

**（B）TNSE 差量 DKG（全员，Phase R）**：
- 留任 922 人仅向新成员发差量份额：$922\times102=94{,}044$ 条
- 新成员向全体 1022 人发差量份额：$102\times1{,}022=104{,}244$ 条
- 小计：$C_{\rm DKG}=198{,}288$ 条

| 方向 | 报文数 |
|------|--------|
| 引导（旧→新，可验证重分享） | 69,768 |
| 留任 922 × 102 新成员（差量） | 94,044 |
| 102 新成员 × 1,022 全员（差量） | 104,244 |
| **合计** | **≈ 268,000** |

对比全量标准 DKG $k(k-1)=1{,}024\times1{,}023\approx1{,}048{,}000$ 条，**减少约 74%**；若仅计 Phase R 差量部分（不含引导），减少约 **81%**。

两部分合并通信量均为 $O(k\cdot k_{\rm new})=O(k^2\cdot\rho_{\rm churn})$，随更替率 $\rho_{\rm churn}\approx10\%$ 线性缩放，而非 $O(k^2)$。$\square$
