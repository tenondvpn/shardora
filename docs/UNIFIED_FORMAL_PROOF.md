...
本方案 DKG 层采用发表于 IEEE TNSE 的 PPKG（Position-Preserving Incremental Key Generation）协议：每 Epoch 仅洗牌约 10% 委员会成员，新节点（10%）执行全量 DKG 广播，复用节点（90%）仅向新节点发送份额、无需与其余复用节点重新交互，全委员会共 $k=1024$ 条广播消息平摊在 10 分钟 Epoch 内约 1.7 条/秒；实测复用节点仅耗约 8 MB 带宽和 17 ms，与 Kronos、sharBFT 相比通信与计算开销均降低 90% 以上（论文 Fig. 5(b)(c)）。正确性与前向安全性详见论文 Claim 3、Claim 4 及定理 13.15。
...
采用 PPKG 协议（10% 洗牌比例，$k_{\mathrm{new}}=102$，$k_{\mathrm{re}}=922$）后：复用节点（90%）仅向 102 个新节点发送份额，每节点通信量削减约 90%；全委员会 $k=1024$ 条广播消息平摊在 600s Epoch 内约 1.7 条/秒，几乎无网络压力；实测复用节点约 8 MB 带宽 + 17 ms 计算，对出块 TPS 无影响；DKG 广播条数恒为 $k$，与候选池规模 $N$ 无关。

| 维度 | 标准全量 DKG | PPKG（10% 洗牌） | 改善 |
|------|------------|----------------|------|
| 广播消息总条数 | $k=1{,}024$ 条 | $k=1{,}024$ 条 | 不变 |
| 消息发送速率 | 突发 | **≈ 1.7 条/秒**（平摊 600s） | 几乎无压力 |
| 复用节点交互对象 | 1023 个 | **102 个（仅新节点）** | **≈ 90% 削减** |
| 复用节点实测带宽 | — | **≈ 8 MB** | 可忽略 |
