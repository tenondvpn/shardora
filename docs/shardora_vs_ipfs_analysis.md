...
数据面采用**优化 1:1 定向推送（Unicast Push）+ 异步按需拉取（Async Pull）**的混合同步策略：

- **推送阶段**：客户端或存储发起节点将 Blob $F$ 以 1:1 定向方式逐节点推送至分片内全部 1024 个节点（非 Gossip 泛洪，每条链路独立传输，避免广播风暴）；
- **异步拉取阶段**：未在推送窗口内完整接收 $F$ 的节点（因网络抖动、短暂离线等），在检测到本地 Blob Pool 缺失对应 CID 后，主动向已接收的对等节点发起异步拉取请求，完成补齐；
