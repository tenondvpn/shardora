# 共识内生存储分片状态机：面向拜占庭容错持久存储的形式化协议设计与分析

*Consensus-Intrinsic Storage Sharding: Formal Protocol Design and Analysis for Byzantine Fault-Tolerant Persistent Storage*

> **核心命题**：本文引入**共识内生存储分片状态机（Consensus-Intrinsic Storage Sharding State Machine，CISSSM）**这一协议原语。在 BFT 故障假设（$f < n/3$，$n = 1024$）下，CISSSM 形式化实现分布式持久存储的数据安全性完备定理 $\mathcal{SEC} = \mathcal{I} \wedge \mathcal{A} \wedge \mathcal{D} \wedge \mathcal{V}$（完整性、可用性、持久性、可验证性），并证明现有代表性方案（IPFS、Filecoin）在可用性与持久性维度存在协议级不完备性。CISSSM 以共识驱动的 1024 节点全副本存储、两层存储证明体系（PoRA + BLS-Store）与 Gas 定价经济学为支柱，在不引入可信第三方的前提下首次实现上述四条安全性质的同时满足，$P_{\text{fail}} \leq 6.5 \times 10^{-16}$（$\beta = 0.2$）。IPFS 与 Filecoin 作为该问题空间的已知局部解，在 §1 中被纳入统一的形式化问题框架进行对比分析。
...
**结论**：Filecoin 在合约期内条件性满足 $\mathcal{A}$ 与 $\mathcal{D}$，但无法在协议层保证 $\mathcal{V}$ 的 $O(1)$ 验证效率（WindowPoSt 验证复杂度 $O(N)$），亦无法满足合约期后的持久性。两者均不满足 $\mathcal{SEC}$ 完备性（定义 E.12，附录 E.2.4）。

---

## 二、Shardora 的核心技术能力

> **学术从属关系（协议原语与参考实现）**：本文提出的 **CISSSM**（Consensus-Intrinsic Storage Sharding State Machine）是一个**抽象协议原语（Abstract Protocol Primitive）**，定义了实现 $\mathcal{SEC}$ 完备性所需的接口规范、故障模型与安全性质——即在 $f < n/3$ 的 BFT 故障假设下同时满足 $\mathcal{I} \wedge \mathcal{A} \wedge \mathcal{D} \wedge \mathcal{V}$（定义 E.12，定理 E.11）。**Shardora** 是 CISSSM 的**具体生产参考实现（Concrete Reference Realization）**，以 HotStuff BFT 为共识内核、以 1024 节点全副本分片存储为状态机，实例化 CISSSM 的全部协议接口。本文附录 E 中各定理的证明对象是 CISSSM 协议原语；Shardora 的具体参数（$n=1024$，$f < n/3$，$k=4$ 等）作为定理的实例化参数出现。二者关系类比于：TLS 协议规范（RFC 8446）与 OpenSSL 具体实现；或 PBFT 协议论文（Castro & Liskov, 1999）与 BFT-SMaRt 实现库。
...
缓解路径：挑战随机数由各 pool 上一轮共识的 BLS 门限聚合签名哈希派生，每轮区块确认后自动更新，无需额外随机源。由于门限签名在 QC 聚合完成前不可预知（BLS 门限不可预测性定理，§12.4 定理1），节点无法预知下一轮挑战目标，"丢弃后恢复"的作弊策略必须在出块间隔（数百毫秒）内完成完整历史区块的网络下载，经济性远不如直接持久存储。长期演进方向是引入简化版时空证明（Proof of Spacetime），对存储的时间连续性进行数学证明。
