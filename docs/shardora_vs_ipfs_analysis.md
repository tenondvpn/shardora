...
**Gas 分配机制**：每笔交易产生的 Gas 费用全额分配给分片内共识节点，不执行销毁。原生代币总量固定，无增发，Gas 收入构成节点维护存储与参与共识的经济激励来源。
...
Shardora 原生代币总量固定为 $\mathcal{S}_{\text{total}} = 2.1 \times 10^{10}$（210 亿），无增发，无销毁，总量恒定。单分片共识吞吐量 $\text{TPS}_{\text{shard}} \approx 10{,}000$，部署 $K$ 个分片时全网吞吐量 $\text{TPS}_{\text{net}} = K \times 10{,}000$。
